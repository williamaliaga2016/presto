...
namespace Multibanca.Backend.Api.Controllers.Multibanca
{
    ...
    [ApiController]
    public class CorregirReparoCopiasEscriturasController : ControllerBase
    {
        private readonly ICorregirReparoCopiasEscriturasApplication CorregirReparoCopiasEscriturasApplicationProvider;
        private readonly IBitacoraApplication BitacoraApplicacionProvider;

        private readonly string ActividadID = Constants.Actividades.CorregirReparoCopiasEscrituras;

        public CorregirReparoCopiasEscriturasController(
            ICorregirReparoCopiasEscriturasApplication _corregirReparoCopiasEscriturasApplication,
            IBitacoraApplication _bitacoraApplication
        )
        {
            CorregirReparoCopiasEscriturasApplicationProvider = _corregirReparoCopiasEscriturasApplication;
        ...
        public async Task<IActionResult> GetByExpediente(long id_expediente)
        {
            ...
            {
                var result =
                    await CorregirReparoCopiasEscriturasApplicationProvider.GetByExpediente(
                        id_expediente
                    );
                ...
                return Ok(new
                {
                    status = true,
                    detail = result,
                    message = "Corregir Reparo Copias de Escrituras obtenido correctamente."
                });
            }
            catch (Exception ex)
            {
        ...
        [HttpPost, Route("Save")]
        public async Task<IActionResult> Save([FromBody] corregir_reparo_copias_escrituras model)
        {
            ...
            try
            {
                ...
                corregir_reparo_copias_escrituras modelSaved;

                if (model.id_corregir_reparo_copias_escrituras == 0)
                {
                    model.row_status = true;
                    model.is_active = true;

                    modelSaved = CorregirReparoCopiasEscriturasApplicationProvider.Create(
                        model,
                        GetUserId()
                    );
                }
                ...
                {
                    modelSaved = CorregirReparoCopiasEscriturasApplicationProvider.Update(
                        model,
                        GetUserId()
                    );
                }
                ...
                corregir_reparo_copias_escrituras? result =
                    await CorregirReparoCopiasEscriturasApplicationProvider.GetByExpediente(
                        modelSaved.id_expediente
                    );
                ...
                response = Ok(new
                {
                    status = true,
                    detail = result,
                    message = "Corregir Reparo Copias de Escrituras guardado correctamente."
                });
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> Avanzar(long id_expediente)
        {
            ...
            try
            {
                ...
                List<AssignActivityDTO> listAssignActividadesDTO =
                    await CorregirReparoCopiasEscriturasApplicationProvider.Avanzar(
                        id_expediente,
                        id_usuario,
                        ActividadID
                    );
