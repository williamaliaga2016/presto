...
namespace Multibanca.Backend.Api.Controllers.Multibanca
{
    ...
    [ApiController]
    public class CorregirReparoCierreCopiasNotariaController : ControllerBase
    {
        private readonly ICorregirReparoCierreCopiasNotariaApplication ApplicationProvider;
        private readonly IBitacoraApplication BitacoraApplicationProvider;

        private readonly string ActividadID = Constants.Actividades.CorregirReparoCierreCopiasNotaria;

        public CorregirReparoCierreCopiasNotariaController(
            ICorregirReparoCierreCopiasNotariaApplication application,
        ...
        public async Task<IActionResult> GetByExpediente(long id_expediente)
        {
            try
            {
                ...
                return Ok(new
                {
                    status = true,
                    detail = result,
                    message = "Corregir Reparo Cierre y Copias Notaria obtenido correctamente."
                });
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> Save(
            [FromBody] corregir_reparo_cierre_copias_notaria model
        )
        {
            ...
            try
            {
                ...
                corregir_reparo_cierre_copias_notaria modelSaved;

                if (model.id_corregir_reparo_cierre_copias_notaria == 0)
                {
                ...
                corregir_reparo_cierre_copias_notaria? result =
                    await ApplicationProvider.GetByExpediente(modelSaved.id_expediente);

                response = Ok(new
                {
                    status = true,
                    detail = result,
                    message = "Corregir Reparo Cierre y Copias Notaria guardado correctamente."
                });
            }
            catch (Exception ex)
            {
