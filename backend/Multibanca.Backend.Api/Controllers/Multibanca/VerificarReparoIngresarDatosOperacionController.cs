...
namespace Multibanca.Backend.Api.Controllers.Multibanca
{
    ...
    [ApiController]
    public class VerificarReparoIngresarDatosOperacionController : ControllerBase
    {
        private readonly IVerificarReparoDatosOperacionApplication VerificarReparoDatosOperacionApplicationProvider;
        private readonly IBitacoraApplication BitacoraApplicacionProvider;

        private readonly string ActividadID = Constants.Actividades.VerificarReparoIngresarDatosOperacion;

        public VerificarReparoIngresarDatosOperacionController(
            IVerificarReparoDatosOperacionApplication _verificarReparoDatosOperacionApplication,
            IBitacoraApplication _bitacoraApplication
        )
        {
            VerificarReparoDatosOperacionApplicationProvider = _verificarReparoDatosOperacionApplication;
        ...
        public async Task<IActionResult> GetByExpediente(long id_expediente)
        {
            ...
            {
                var result =
                    await VerificarReparoDatosOperacionApplicationProvider.GetByExpediente(
                        id_expediente
                    );
                ...
                return Ok(new
                {
                    status = true,
                    detail = result,
                    message = "Verificar Reparo Ingresar Datos Operación obtenido correctamente."
                });
            }
            catch (Exception ex)
            {
        ...
        [HttpPost, Route("Save")]
        public async Task<IActionResult> Save([FromBody] verificar_reparo_datos_operacion model)
        {
            ...
            try
            {
                ...
                verificar_reparo_datos_operacion modelSaved;

                if (model.id_verificar_reparo_datos_operacion == 0)
                {
                    model.row_status = true;
                    model.is_active = true;

                    modelSaved = VerificarReparoDatosOperacionApplicationProvider.Create(
                        model,
                        GetUserId()
                    );
                }
                ...
                {
                    modelSaved = VerificarReparoDatosOperacionApplicationProvider.Update(
                        model,
                        GetUserId()
                    );
                }
                ...
                verificar_reparo_datos_operacion? result =
                    await VerificarReparoDatosOperacionApplicationProvider.GetByExpediente(
                        modelSaved.id_expediente
                    );
                ...
                response = Ok(new
                {
                    status = true,
                    detail = result,
                    message = "Verificar Reparo Ingresar Datos Operación guardado correctamente."
                });
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> Avanzar(long id_expediente)
        {
            ...
            try
            {
                ...
                List<AssignActivityDTO> listAssignActividadesDTO =
                    await VerificarReparoDatosOperacionApplicationProvider.Avanzar(
                        id_expediente,
                        id_usuario,
                        ActividadID
                    );
