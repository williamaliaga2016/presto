...
namespace Multibanca.Backend.Api.Controllers.Multibanca
{
    ...
    [ApiController]
    public class CorregirCartaResguardoController : ControllerBase
    {
        private readonly ICorregirCartaResguardoApplication CorregirCartaResguardoApplicationProvider;
        private readonly IBitacoraApplication BitacoraApplicacionProvider;

        private readonly string ActividadID = Constants.Actividades.CorregirCartaResguardo;

        public CorregirCartaResguardoController(
            ICorregirCartaResguardoApplication _corregirCartaResguardoApplication,
            IBitacoraApplication _bitacoraApplication
        )
        {
            CorregirCartaResguardoApplicationProvider = _corregirCartaResguardoApplication;
        ...
        public async Task<IActionResult> GetByExpediente(long id_expediente)
        {
            ...
            {
                var result =
                    await CorregirCartaResguardoApplicationProvider.GetByExpediente(
                        id_expediente
                    );
                ...
                return Ok(new
                {
                    status = true,
                    detail = result,
                    message = "Corregir Carta Resguardo obtenido correctamente."
                });
            }
            catch (Exception ex)
            {
        ...
        [HttpPost, Route("Save")]
        public async Task<IActionResult> Save([FromBody] corregir_carta_resguardo model)
        {
            ...
            try
            {
                ...
                corregir_carta_resguardo modelSaved;

                if (model.id_corregir_carta_resguardo == 0)
                {
                    model.row_status = true;
                    model.is_active = true;

                    modelSaved = CorregirCartaResguardoApplicationProvider.Create(
                        model,
                        GetUserId()
                    );
                }
                ...
                {
                    modelSaved = CorregirCartaResguardoApplicationProvider.Update(
                        model,
                        GetUserId()
                    );
                }
                ...
                corregir_carta_resguardo? result =
                    await CorregirCartaResguardoApplicationProvider.GetByExpediente(
                        modelSaved.id_expediente
                    );
                ...
                response = Ok(new
                {
                    status = true,
                    detail = result,
                    message = "Corregir Carta Resguardo guardado correctamente."
                });
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> Avanzar(long id_expediente)
        {
            ...
            try
            {
                ...
                List<AssignActivityDTO> listAssignActividadesDTO =
                    await CorregirCartaResguardoApplicationProvider.Avanzar(
                        id_expediente,
                        id_usuario,
                        ActividadID
                    );
