...
namespace Multibanca.Backend.Api.Controllers.Multibanca
{
    ...
    [ApiController]
    public class CorregirControlEscrituraController : ControllerBase
    {
        private readonly ICorregirControlEscrituraApplication CorregirControlEscrituraApplicationProvider;
        private readonly IBitacoraApplication BitacoraApplicacionProvider;

        private readonly string ActividadID = Constants.Actividades.CorregirControlEscritura;

        public CorregirControlEscrituraController(
            ICorregirControlEscrituraApplication _corregirControlEscrituraApplication,
            IBitacoraApplication _bitacoraApplication,
            IWorkflowApplication _workflowApplication)
        {
            CorregirControlEscrituraApplicationProvider = _corregirControlEscrituraApplication;
        ...
        public async Task<IActionResult> GetByExpediente(long idExpediente)
        {
            try
            {
                var result = await CorregirControlEscrituraApplicationProvider.GetByExpediente(idExpediente);

                return Ok(new
                {
                    status = true,
                    detail = result,
                    message = "Corregir Control Escritura obtenido correctamente."
                });
            }
            catch (Exception ex)
            {
        ...
        [HttpPost, Route("Save")]
        public async Task<IActionResult> Save([FromBody] corregir_control_escritura model)
        {
            ...
            try
            {
                ...
                corregir_control_escritura modelSaved;

                if (model.id_corregir_control_escritura == 0)
                {
                    model.row_status = true;
                    model.is_active = true;

                    modelSaved = CorregirControlEscrituraApplicationProvider.Create(
                        model,
                        GetUserId()
                    );
                }
                ...
                {
                    modelSaved = CorregirControlEscrituraApplicationProvider.Update(
                        model,
                        GetUserId()
                    );
                }
                ...
                corregir_control_escritura? result =
                    await CorregirControlEscrituraApplicationProvider.GetByExpediente(
                        modelSaved.id_expediente
                    );
                ...
                response = Ok(new
                {
                    status = true,
                    detail = result,
                    message = "Corregir Control Escritura guardado correctamente."
                });
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> Avanzar(long id_expediente)
        {
            ...
            try
            {
                ...
                List<AssignActivityDTO> listAssignActividadesDTO =
                    await CorregirControlEscrituraApplicationProvider.Avanzar(
                        id_expediente,
                        id_usuario,
                        ActividadID
                    );
