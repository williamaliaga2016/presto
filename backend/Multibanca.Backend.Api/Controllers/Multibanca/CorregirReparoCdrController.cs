...
namespace Multibanca.Backend.Api.Controllers.Multibanca
{
    ...
    [ApiController]
    public class CorregirReparoCdrController : ControllerBase
    {
        private readonly ICorregirReparoCdrApplication CorregirReparoCdrApplicationProvider;
        private readonly IBitacoraApplication BitacoraApplicacionProvider;

        private readonly string ActividadID = Constants.Actividades.CorregirReparoCdr;

        public CorregirReparoCdrController(
            ICorregirReparoCdrApplication _corregirReparoCdrApplication,
            IBitacoraApplication _bitacoraApplication
        )
        {
            CorregirReparoCdrApplicationProvider = _corregirReparoCdrApplication;
        ...
        public async Task<IActionResult> GetByExpediente(long id_expediente)
        {
            ...
            {
                var result =
                    await CorregirReparoCdrApplicationProvider.GetByExpediente(
                        id_expediente
                    );
                ...
                return Ok(new
                {
                    status = true,
                    detail = result,
                    message = "Corregir Reparo CdR obtenido correctamente."
                });
            }
            catch (Exception ex)
            {
        ...
        [HttpPost, Route("Save")]
        public async Task<IActionResult> Save([FromBody] corregir_reparo_cdr model)
        {
            ...
            try
            {
                ...
                corregir_reparo_cdr modelSaved;

                if (model.id_corregir_reparo_cdr == 0)
                {
                    model.row_status = true;
                    model.is_active = true;

                    modelSaved = CorregirReparoCdrApplicationProvider.Create(
                        model,
                        GetUserId()
                    );
                }
                ...
                {
                    modelSaved = CorregirReparoCdrApplicationProvider.Update(
                        model,
                        GetUserId()
                    );
                }
                ...
                response = Ok(new
                {
                    status = true,
                    detail = result,
                    message = "Corregir Reparo CdR guardado correctamente."
                });
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> Avanzar(long id_expediente)
        {
            ...
            try
            {
                ...
                List<AssignActivityDTO> listAssignActividadesDTO =
                    await CorregirReparoCdrApplicationProvider.Avanzar(
                        id_expediente,
                        id_usuario,
                        ActividadID
                    );
