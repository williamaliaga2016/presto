...
namespace Multibanca.Backend.Api.Controllers.Multibanca
{
    ...
    [ApiController]
    public class ReparoFormularioController : ControllerBase
    {
        private readonly IReparoFormularioApplication ReparoFormularioApplicationProvider;
        private readonly IBitacoraApplication BitacoraApplicacionProvider;

        private readonly string ActividadID = Constants.Actividades.ReparoFormulario;

        public ReparoFormularioController(
            IReparoFormularioApplication _reparoFormularioApplication,
            IBitacoraApplication _bitacoraApplication
        )
        {
            ReparoFormularioApplicationProvider = _reparoFormularioApplication;
        ...
        public async Task<IActionResult> GetByExpediente(long id_expediente)
        {
            try
            {
                var result = await ReparoFormularioApplicationProvider.GetByExpediente(
                    id_expediente
                );
                ...
                return Ok(new
                {
                    status = true,
                    detail = result,
                    message = "Reparo Formulario obtenido correctamente."
                });
            }
            catch (Exception ex)
            {
        ...
        [HttpPost, Route("Save")]
        public async Task<IActionResult> Save([FromBody] reparo_formulario model)
        {
            ...
            try
            {
                ...
                reparo_formulario modelSaved;

                if (model.id_reparo_formulario == 0)
                {
                    model.row_status = true;
                    model.is_active = true;

                    modelSaved = ReparoFormularioApplicationProvider.Create(
                        model,
                        GetUserId()
                    );
                }
                ...
                {
                    modelSaved = ReparoFormularioApplicationProvider.Update(
                        model,
                        GetUserId()
                    );
                }
                ...
                reparo_formulario? result =
                    await ReparoFormularioApplicationProvider.GetByExpediente(
                        modelSaved.id_expediente
                    );
                ...
                response = Ok(new
                {
                    status = true,
                    detail = result,
                    message = "Reparo Formulario guardado correctamente."
                });
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> Avanzar(long id_expediente)
        {
            ...
            try
            {
                ...
                List<AssignActivityDTO> listAssignActividadesDTO =
                    await ReparoFormularioApplicationProvider.Avanzar(
                        id_expediente,
                        id_usuario,
                        ActividadID
                    );
