...
namespace Multibanca.Backend.Api.Controllers.Multibanca
{
    ...
    [ApiController]
    public class ReingresarEscrituraCbrController : ControllerBase
    {
        private readonly IReingresarEscrituraCbrApplication ReingresarEscrituraCbrApplicationProvider;
        private readonly IBitacoraApplication BitacoraApplicacionProvider;

        private readonly string ActividadID = Constants.Actividades.ReingresarEscrituraCbr;

        public ReingresarEscrituraCbrController(
            IReingresarEscrituraCbrApplication _reingresarEscrituraCbrApplication,
            IBitacoraApplication _bitacoraApplication
        )
        {
            ReingresarEscrituraCbrApplicationProvider = _reingresarEscrituraCbrApplication;
        ...
        public async Task<IActionResult> GetByExpediente(long id_expediente)
        {
            ...
            {
                var result =
                    await ReingresarEscrituraCbrApplicationProvider.GetByExpediente(
                        id_expediente
                    );
                ...
                return Ok(new
                {
                    status = true,
                    detail = result,
                    message = "Reingresar Escritura en CBR obtenido correctamente."
                });
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> Save(
            [FromBody] reingresar_escritura_cbr model
        )
        {
            ...
            try
            {
                ...
                reingresar_escritura_cbr modelSaved;

                if (model.id_reingresar_escritura_cbr == 0)
                {
                    model.row_status = true;
                    model.is_active = true;

                    modelSaved = ReingresarEscrituraCbrApplicationProvider.Create(
                        model,
                        GetUserId()
                    );
                }
                ...
                {
                    modelSaved = ReingresarEscrituraCbrApplicationProvider.Update(
                        model,
                        GetUserId()
                    );
                }
                ...
                reingresar_escritura_cbr? result =
                    await ReingresarEscrituraCbrApplicationProvider.GetByExpediente(
                        modelSaved.id_expediente
                    );
                ...
                response = Ok(new
                {
                    status = true,
                    detail = result,
                    message = "Reingresar Escritura en CBR guardado correctamente."
                });
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> Avanzar(long id_expediente)
        {
            ...
            try
            {
                ...
                List<AssignActivityDTO> listAssignActividadesDTO =
                    await ReingresarEscrituraCbrApplicationProvider.Avanzar(
                        id_expediente,
                        id_usuario,
                        ActividadID
                    );
