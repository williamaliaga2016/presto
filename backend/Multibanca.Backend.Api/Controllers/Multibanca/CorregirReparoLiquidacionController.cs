...
namespace Multibanca.Backend.Api.Controllers.Multibanca
{
    ...
    [ApiController]
    public class CorregirReparoLiquidacionController : ControllerBase
    {
        private readonly ICorregirReparoLiquidacionApplication CorregirReparoLiquidacionApplicationProvider;
        private readonly IBitacoraApplication BitacoraApplicacionProvider;

        private readonly string ActividadID = Constants.Actividades.CorregirReparoLiquidacion;

        public CorregirReparoLiquidacionController(
            ICorregirReparoLiquidacionApplication _corregirReparoLiquidacionApplication,
            IBitacoraApplication _bitacoraApplication
        )
        {
            CorregirReparoLiquidacionApplicationProvider = _corregirReparoLiquidacionApplication;
        ...
        public async Task<IActionResult> GetByExpediente(long id_expediente)
        {
            ...
            {
                var result =
                    await CorregirReparoLiquidacionApplicationProvider.GetByExpediente(
                        id_expediente
                    );
                ...
                return Ok(new
                {
                    status = true,
                    detail = result,
                    message = "Corregir Reparo Liquidación obtenido correctamente."
                });
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> Save(
            [FromBody] corregir_reparo_liquidacion model
        )
        {
            ...
            try
            {
                ...
                corregir_reparo_liquidacion modelSaved;

                if (model.id_corregir_reparo_liquidacion == 0)
                {
                    model.row_status = true;
                    model.is_active = true;

                    modelSaved = CorregirReparoLiquidacionApplicationProvider.Create(
                        model,
                        GetUserId()
                    );
                }
                ...
                {
                    modelSaved = CorregirReparoLiquidacionApplicationProvider.Update(
                        model,
                        GetUserId()
                    );
                }
                ...
                corregir_reparo_liquidacion? result =
                    await CorregirReparoLiquidacionApplicationProvider.GetByExpediente(
                        modelSaved.id_expediente
                    );
                ...
                response = Ok(new
                {
                    status = true,
                    detail = result,
                    message = "Corregir Reparo Liquidación guardado correctamente."
                });
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> Avanzar(long id_expediente)
        {
            ...
            try
            {
                ...
                List<AssignActivityDTO> listAssignActividadesDTO =
                    await CorregirReparoLiquidacionApplicationProvider.Avanzar(
                        id_expediente,
                        id_usuario,
                        ActividadID
                    );
