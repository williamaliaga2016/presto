...
namespace Multibanca.Backend.Api.Controllers.Multibanca
{
    ...
    [ApiController]
    public class CorregirReparoVisadoController : ControllerBase
    {
        private readonly ICorregirReparoVisadoApplication CorregirReparoVisadoApplicationProvider;
        private readonly IBitacoraApplication BitacoraApplicacionProvider;

        private readonly string ActividadID = Constants.Actividades.CorregirReparoVisado;

        public CorregirReparoVisadoController(
            ICorregirReparoVisadoApplication _corregirReparoVisadoApplication,
            IBitacoraApplication _bitacoraApplication
        )
        {
            CorregirReparoVisadoApplicationProvider = _corregirReparoVisadoApplication;
        ...
        public async Task<IActionResult> GetByExpediente(long id_expediente)
        {
            try
            {
                var result = await CorregirReparoVisadoApplicationProvider.GetByExpediente(
                    id_expediente
                );
                ...
                return Ok(new
                {
                    status = true,
                    detail = result,
                    message = "Corregir Reparo Visado obtenido correctamente."
                });
            }
            catch (Exception ex)
            {
        ...
        [HttpPost, Route("Save")]
        public async Task<IActionResult> Save([FromBody] corregir_reparo_visado model)
        {
            ...
            try
            {
                ...
                corregir_reparo_visado modelSaved;

                if (model.id_corregir_reparo_visado == 0)
                {
                    model.row_status = true;
                    model.is_active = true;

                    modelSaved = CorregirReparoVisadoApplicationProvider.Create(
                        model,
                        GetUserId()
                    );
                }
                ...
                {
                    modelSaved = CorregirReparoVisadoApplicationProvider.Update(
                        model,
                        GetUserId()
                    );
                }
                ...
                corregir_reparo_visado? result =
                    await CorregirReparoVisadoApplicationProvider.GetByExpediente(
                        modelSaved.id_expediente
                    );
                ...
                response = Ok(new
                {
                    status = true,
                    detail = result,
                    message = "Corregir Reparo Visado guardado correctamente."
                });
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> Avanzar(long id_expediente)
        {
            ...
            try
            {
                ...
                List<AssignActivityDTO> listAssignActividadesDTO =
                    await CorregirReparoVisadoApplicationProvider.Avanzar(
                        id_expediente,
                        id_usuario,
                        ActividadID
                    );
