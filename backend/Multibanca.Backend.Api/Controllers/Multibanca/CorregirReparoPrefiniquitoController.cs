...
namespace Multibanca.Backend.Api.Controllers.Multibanca
{
    ...
    [ApiController]
    public class CorregirReparoPrefiniquitoController : ControllerBase
    {
        private readonly ICorregirReparoPrefiniquitoApplication CorregirReparoPrefiniquitoApplicationProvider;
        private readonly IBitacoraApplication BitacoraApplicacionProvider;

        private readonly string ActividadID = Constants.Actividades.CorregirReparoPrefiniquito;

        public CorregirReparoPrefiniquitoController(
            ICorregirReparoPrefiniquitoApplication _corregirReparoPrefiniquitoApplication,
            IBitacoraApplication _bitacoraApplication
        )
        {
            CorregirReparoPrefiniquitoApplicationProvider = _corregirReparoPrefiniquitoApplication;
        ...
        public async Task<IActionResult> GetByExpediente(long id_expediente)
        {
            try
            {
                corregir_reparo_prefiniquito? result =
                    await CorregirReparoPrefiniquitoApplicationProvider.GetByExpediente(
                        id_expediente
                    );
                ...
                return Ok(new
                {
                    status = true,
                    detail = result,
                    message = "Corregir Reparo Prefiniquito obtenido correctamente."
                });
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> Save(
            [FromBody] corregir_reparo_prefiniquito model
        )
        {
            ...
            try
            {
                ...
                corregir_reparo_prefiniquito modelSaved;

                if (model.id_corregir_reparo_prefiniquito == 0)
                {
                    model.row_status = true;
                    model.is_active = true;

                    modelSaved = CorregirReparoPrefiniquitoApplicationProvider.Create(
                        model,
                        GetUserId()
                    );
                }
                ...
                {
                    modelSaved = CorregirReparoPrefiniquitoApplicationProvider.Update(
                        model,
                        GetUserId()
                    );
                }
                ...
                response = Ok(new
                {
                    status = true,
                    detail = result,
                    message = "Corregir Reparo Prefiniquito guardado correctamente."
                });
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> Avanzar(long id_expediente)
        {
            ...
            try
            {
                ...
                List<AssignActivityDTO> listAssignActividadesDTO =
                    await CorregirReparoPrefiniquitoApplicationProvider.Avanzar(
                        id_expediente,
                        id_usuario,
                        ActividadID
                    );
