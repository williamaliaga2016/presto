...
namespace Multibanca.Backend.Api.Controllers.Multibanca
{
    ...
    [ApiController]
    public class ValidacionRectificatoriaLegalPostventaController : ControllerBase
    {
        private readonly IValidacionRectificatoriaLegalPostventaApplication ValidacionRectificatoriaLegalPostventaApplicationProvider;
        ...
        private readonly string ActividadID = Constants.Actividades.ValidacionRectificatoriaLegalPostventa;

        public ValidacionRectificatoriaLegalPostventaController(
            IValidacionRectificatoriaLegalPostventaApplication _ValidacionRectificatoriaLegalPostventaApplication,
        ...
        {
            ValidacionRectificatoriaLegalPostventaApplicationProvider = _ValidacionRectificatoriaLegalPostventaApplication;
        ...
        public async Task<IActionResult> GetByExpediente(long idExpediente)
        {
            try
            {
                var result = await ValidacionRectificatoriaLegalPostventaApplicationProvider.GetByExpediente(idExpediente);
        ...
        public async Task<IActionResult> GetFaltantesFirmaByExpediente(long idExpediente)
        {
            try
            {
                var result = await ValidacionRectificatoriaLegalPostventaDatosPersonalesApplicationProvider.GetByExpediente(idExpediente);
        ...
        [HttpPost, Route("Save")]
        public async Task<IActionResult> Save([FromBody] validacion_rectificatoria_legal_postventa validacion_rectificatoria_legal_postventa)
        {
            {
                ...
                try
                {
                    if (validacion_rectificatoria_legal_postventa.id_usuario_solicitante == 0)
                    {
                    ...
                    if (validacion_rectificatoria_legal_postventa.is_subsanar && string.IsNullOrWhiteSpace(validacion_rectificatoria_legal_postventa.observaciones))
                    {
                        return BadRequest(new
                        {
                            status = false,
                    ...
                    validacion_rectificatoria_legal_postventa modelSaved;

                    if (validacion_rectificatoria_legal_postventa.id_validacion_rectificatoria_legal_postventa == 0)
                    {
                        validacion_rectificatoria_legal_postventa.row_status = true;
                        validacion_rectificatoria_legal_postventa.is_active = true;

                        modelSaved = ValidacionRectificatoriaLegalPostventaApplicationProvider.Create(
                            validacion_rectificatoria_legal_postventa,
                            GetUserId()
                        );
                    }
                    else
                    {
                        modelSaved = ValidacionRectificatoriaLegalPostventaApplicationProvider.Update(
                            validacion_rectificatoria_legal_postventa,
                            GetUserId()
                        );
                    }
                    ...
                    if (validacion_rectificatoria_legal_postventa.validacion_rectificatoria_legal_postventa_datos_personales != null)
                    {
                        foreach (var faltante_firma in validacion_rectificatoria_legal_postventa.validacion_rectificatoria_legal_postventa_datos_personales)
                        {
                            faltante_firma.id_validacion_rectificatoria_legal_postventa = modelSaved.id_validacion_rectificatoria_legal_postventa;
                            faltante_firma.id_expediente = modelSaved.id_expediente;

                            if (faltante_firma.id_validacion_rectificatoria_legal_postventa_datos_personales == 0)
                            {
                                faltante_firma.is_active = true;
                                faltante_firma.row_status = true;

                                ValidacionRectificatoriaLegalPostventaDatosPersonalesApplicationProvider.Create(faltante_firma, GetUserId());
                            }
                            else
                            {
                                ValidacionRectificatoriaLegalPostventaDatosPersonalesApplicationProvider.Update(faltante_firma, GetUserId());
                            }
                        }
                    }

                    var resultBitacora = await BitacoraApplicacionProvider.GetByExpedienteActividad(validacion_rectificatoria_legal_postventa.id_expediente, ActividadID);

                    bitacora bitacora = new bitacora();
                    bitacora.id_expediente = validacion_rectificatoria_legal_postventa.id_expediente;
                    ...
                    validacion_rectificatoria_legal_postventa? result = await ValidacionRectificatoriaLegalPostventaApplicationProvider
        ...
        public async Task<IActionResult> Avanzar(long id_expediente)
        {
            ...
            try
            {
                ...
                List<AssignActivityDTO> listAssignActividadesDTO =
                    await ValidacionRectificatoriaLegalPostventaApplicationProvider.Avanzar(id_expediente, id_usuario, ActividadID);
        ...
        [HttpGet, Route("GetControlesValidacionRectificatoriaLegalPostventa")]
        public async Task<IActionResult> GetControlesValidacionRectificatoriaLegalPostventa()
        {
            try
            {
