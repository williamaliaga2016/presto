...
[Authorize]
[ApiController]
[Route("api/realizar-entrega-ep-firmada")]
public class RealizarEntregaEpFirmadaController : ControllerBase
{
    private readonly IRealizarEntregaEpFirmadaApplication ApplicationProvider;
    private readonly string ActividadID = Constants.ActividadesBBVA.EscrituracionRealizarEntregaEpFirmada;

    public RealizarEntregaEpFirmadaController(IRealizarEntregaEpFirmadaApplication application)
    {
    ...
    public async Task<IActionResult> GetByIdExpediente(long id_expediente)
    {
        try
        {
            ...
            return Ok(new { status = true, detail = result, message = "Realizar Entrega EP Firmada consultado correctamente." });
        }
        catch (Exception ex)
        {
    ...
    [HttpPost("Save")]
    public IActionResult Save([FromBody] realizar_entrega_ep_firmada model)
    {
        try
        {
            ...
            return Ok(new { status = true, detail = model, message = "Realizar Entrega EP Firmada guardado correctamente." });
        }
        catch (Exception ex)
        {
            return HandleException(ex);
