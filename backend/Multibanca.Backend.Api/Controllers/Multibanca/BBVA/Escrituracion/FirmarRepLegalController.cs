...
[Authorize]
[ApiController]
[Route("api/firmar-rep-legal")]
public class FirmarRepLegalController : ControllerBase
{
    private readonly IFirmarRepLegalApplication ApplicationProvider;
    private readonly string ActividadID = Constants.ActividadesBBVA.EscrituracionFirmarRepLegal;

    public FirmarRepLegalController(IFirmarRepLegalApplication application)
    {
    ...
    public async Task<IActionResult> GetByIdExpediente(long id_expediente)
    {
        try
        {
            ...
            return Ok(new { status = true, detail = result, message = "Firmar Rep. Legal consultado correctamente." });
        }
        catch (Exception ex)
        {
    ...
    [HttpPost("Save")]
    public IActionResult Save([FromBody] firmar_rep_legal model)
    {
        try
        {
            ...
            return Ok(new { status = true, detail = model, message = "Firmar Rep. Legal guardado correctamente." });
        }
        catch (Exception ex)
        {
            return HandleException(ex);
