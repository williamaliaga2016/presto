...
[Authorize]
[ApiController]
[Route("api/realizar-devolucion-ep")]
public class RealizarDevolucionEPController : ControllerBase
{
    private readonly IRealizarDevolucionEPApplication ApplicationProvider;
    private readonly string ActividadID = Constants.ActividadesBBVA.EscrituracionRealizarDevolucionEP;

    public RealizarDevolucionEPController(IRealizarDevolucionEPApplication application)
    {
    ...
    public async Task<IActionResult> GetByIdExpediente(long id_expediente)
    {
        try
        {
            ...
            return Ok(new { status = true, detail = result, message = "Realizar Devolución EP consultado correctamente." });
        }
    ...
    [HttpPost("Save")]
    public IActionResult Save([FromBody] realizar_devolucion_ep model)
    {
        try
        {
            ...
            if (model.id == 0) { model.row_status = true; model.is_active = true; model = ApplicationProvider.Create(model, GetUserId()); }
            else { model.row_status = true; model.is_active = true; model = ApplicationProvider.Update(model, GetUserId()); }
            return Ok(new { status = true, detail = model, message = "Realizar Devolución EP guardado correctamente." });
        }
        catch (Exception ex) { return HandleException(ex); }
    }
    ...
    public async Task<IActionResult> Avanzar(long id_expediente)
    {
        try
        {
            ...
            if (result.Count > 0) return Ok(new { status = true, detail = result, message = "Actividad avanzada correctamente." });
