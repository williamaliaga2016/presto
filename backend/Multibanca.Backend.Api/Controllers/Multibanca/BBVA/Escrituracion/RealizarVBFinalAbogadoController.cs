...
[Authorize]
[ApiController]
[Route("api/realizar-vb-final-abogado")]
public class RealizarVBFinalAbogadoController : ControllerBase
{
    private readonly IRealizarVBFinalAbogadoApplication ApplicationProvider;
    private readonly string ActividadID = Constants.ActividadesBBVA.EscrituracionRealizarVBFinalAbogado;

    public RealizarVBFinalAbogadoController(IRealizarVBFinalAbogadoApplication application)
    {
    ...
    public async Task<IActionResult> GetByIdExpediente(long id_expediente)
    {
        try
        {
            ...
            return Ok(new { status = true, detail = result, message = "Realizar VB Final Abogado consultado correctamente." });
        }
    ...
    [HttpPost("Save")]
    public IActionResult Save([FromBody] realizar_vb_final_abogado model)
    {
        try
        {
            ...
            return Ok(new { status = true, detail = model, message = "Realizar VB Final Abogado guardado correctamente." });
        }
        catch (Exception ex) { return HandleException(ex); }
    }
