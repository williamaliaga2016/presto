...
[Authorize]
[ApiController]
[Route("api/validar-condiciones-desembolso")]
public class ValidarCondicionesDesembolsoController : ControllerBase
{
    private readonly IValidarCondicionesDesembolsoApplication ApplicationProvider;
    private readonly string ActividadID = Constants.ActividadesBBVA.EscrituracionValidarCondicionesDesembolso;

    public ValidarCondicionesDesembolsoController(IValidarCondicionesDesembolsoApplication application)
    {
    ...
    public async Task<IActionResult> GetByIdExpediente(long id_expediente)
    {
        try
        {
            ...
            return Ok(new { status = true, detail = result, message = "Validar Condiciones Desembolso consultado correctamente." });
        }
        catch (Exception ex) { return HandleException(ex); }
    }

    [HttpPost("Save")]
    public IActionResult Save([FromBody] validar_condiciones_desembolso model)
    {
        try
        {
            ...
            return Ok(new { status = true, detail = model, message = "Validar Condiciones Desembolso guardado correctamente." });
        }
        catch (Exception ex) { return HandleException(ex); }
    }
    ...
    [HttpPost("suspender/{id_expediente}")]
    public async Task<IActionResult> Suspender(long id_expediente)
    {
        try
        {
            var userId = GetUserId();
            var result = await ApplicationProvider.Suspender(id_expediente, userId);
            return Ok(new { status = true, detail = result, message = "Actividad suspendida correctamente." });
        }
        catch (Exception ex) { return HandleException(ex); }
    }
