...
{
    public class CorregirReparoCopiasEscriturasApplication
        : MultibancaGenericApplication<corregir_reparo_copias_escrituras, corregir_reparo_copias_escrituras_entity, ICorregirReparoCopiasEscriturasRepository>,
          ICorregirReparoCopiasEscriturasApplication
    {
        private readonly ICorregirReparoCopiasEscriturasRepository CorregirReparoCopiasEscriturasRepositoryProvider;
        ...
        public CorregirReparoCopiasEscriturasApplication(
            MultibancaDBContext _multibancaDBContext,
            ICorregirReparoCopiasEscriturasRepository _corregirReparoCopiasEscriturasRepository,
            IRevisarCopiasEscriturasApplication _revisarCopiasEscriturasApplication,
            IUserApplication _userApplication,
            IWorkflowApplication _workflowApplication,
            IMapper _mapper
        ) : base(_multibancaDBContext, _corregirReparoCopiasEscriturasRepository, _mapper)
        {
            CorregirReparoCopiasEscriturasRepositoryProvider = _corregirReparoCopiasEscriturasRepository;
        ...
        public async Task<corregir_reparo_copias_escrituras?> GetByExpediente(long id_expediente)
        {
            corregir_reparo_copias_escrituras_entity? entity =
                await CorregirReparoCopiasEscriturasRepositoryProvider.GetByExpediente(id_expediente);

            corregir_reparo_copias_escrituras domain =
                Mapper.Map<corregir_reparo_copias_escrituras?>(entity)
                ?? new corregir_reparo_copias_escrituras();
        ...
        {
            var entity =
                await CorregirReparoCopiasEscriturasRepositoryProvider.GetByExpediente(
                    expediente_id
                );
            ...
            if (entity == null)
            {
                throw new InvalidOperationException(
                    $"No existe registro de Corregir Reparo Copias de Escrituras para el expediente {expediente_id}."
                );
            }
            ...
            string transitionsID = listTransitions
                .Where(x => x.name == "CorregirReparoCopiasEscritura_ValorizarCBR")
            ...
            {
                throw new InvalidOperationException(
                    "No se encontró transición configurada para Corregir Reparo Copias de Escrituras."
                );
            }
        ...
        {
            revisar_copias_escrituras? actividadOrigen =
                await RevisarCopiasEscriturasApplicationProvider.GetByExpediente(
                    id_expediente
                );
