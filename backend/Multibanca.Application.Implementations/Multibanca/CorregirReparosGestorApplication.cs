...
{
    public class CorregirReparosGestorApplication
        : MultibancaGenericApplication<corregir_reparos_gestor, corregir_reparos_gestor_entity, ICorregirReparosGestorRepository>,
          ICorregirReparosGestorApplication
    {
        private readonly ICorregirReparosGestorRepository CorregirReparosGestorRepositoryProvider;
        ...
        public CorregirReparosGestorApplication(
            MultibancaDBContext _multibancaDBContext,
            ICorregirReparosGestorRepository _corregirReparosGestorRepository,
            IVerificarReparoCbrApplication _verificarReparoCbrApplication,
            IUserApplication _userApplication,
            IWorkflowApplication _workflowApplication,
            IMapper _mapper
        ) : base(_multibancaDBContext, _corregirReparosGestorRepository, _mapper)
        {
            CorregirReparosGestorRepositoryProvider = _corregirReparosGestorRepository;
        ...
        public async Task<corregir_reparos_gestor?> GetByExpediente(long id_expediente)
        {
            var entity = await CorregirReparosGestorRepositoryProvider.GetByExpediente(id_expediente);

            var domain = Mapper.Map<corregir_reparos_gestor?>(entity)
                ?? new corregir_reparos_gestor();
        ...
        {
            var entity = await CorregirReparosGestorRepositoryProvider.GetByExpediente(expediente_id);

            if (entity == null)
            {
                throw new InvalidOperationException(
                    $"No existe registro de Corregir Reparos por Gestor para el expediente {expediente_id}."
                );
            }
            ...
            //Comentado por Luigi, revisar requerimiento 
            //if (string.IsNullOrWhiteSpace(entity.estatus_general))
            //{
            //    throw new InvalidOperationException(
            //        "El campo Estatus General es obligatorio para avanzar."
            //    );
            //}
            ...
            {
                throw new InvalidOperationException(
                    "No se encontró transición configurada para Corregir Reparos por Gestor."
                );
            }
        ...
        private async Task CompletarDatosHeredados(
            corregir_reparos_gestor domain,
            long id_expediente
        )
        {
            verificar_reparo_cbr? actividadOrigen =
                await VerificarReparoCbrApplicationProvider.GetByExpediente(id_expediente);
