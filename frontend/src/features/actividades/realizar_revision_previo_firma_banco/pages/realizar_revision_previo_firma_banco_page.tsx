...
import { useAvanzarRealizarRevisionPrevioFirmaBanco } from "../hooks/useAvanzarRealizarRevisionPrevioFirmaBanco";
import { useRealizarRevisionPrevioFirmaBanco } from "../hooks/useRealizarRevisionPrevioFirmaBanco";
import { useUpsertRealizarRevisionPrevioFirmaBanco } from "../hooks/useUpsertRealizarRevisionPrevioFirmaBanco";

const ACTIVITY_ID = "_Lw2Q_M8rBebJp3_R7nD5t";

const buildInitialState = (id_expediente: number): RealizarRevisionPrevioFirmaBanco => ({
  id_realizar_revision_previo_firma_banco: 0,
  id_expediente,
...
const normalizeRealizarRevisionPrevioFirmaBanco = (
  source: Partial<RealizarRevisionPrevioFirmaBanco> | null | undefined,
  id_expediente_fallback: number,
): RealizarRevisionPrevioFirmaBanco => {
  return {
    id_realizar_revision_previo_firma_banco: Number(source?.id_realizar_revision_previo_firma_banco ?? 0),
...
export default function RealizarRevisionPrevioFirmaBancoPage() {
  ...
  const [form, setForm] = useState<RealizarRevisionPrevioFirmaBanco>(
  ...
  const { data, isLoading } = useRealizarRevisionPrevioFirmaBanco(id_expediente);
  const saveMutation = useUpsertRealizarRevisionPrevioFirmaBanco();
  const avanzarMutation = useAvanzarRealizarRevisionPrevioFirmaBanco();
  ...
  useEffect(() => {
    ...
    if (data?.status && data.detail) {
      const loadedEntity = normalizeRealizarRevisionPrevioFirmaBanco(
        data.detail,
        id_expediente,
      );

      setForm(loadedEntity);
      setIsDisabled(Number(data.detail.id_realizar_revision_previo_firma_banco) > 0);
      setCanAdvance(false);
    ...
    if (data) {
      setForm((prev) =>
        normalizeRealizarRevisionPrevioFirmaBanco(
          {
            ...prev,
            id_expediente,
          },
  ...
  const updateField = <K extends keyof RealizarRevisionPrevioFirmaBanco>(
    field: K,
    value: RealizarRevisionPrevioFirmaBanco[K],
  ) => {
    setForm((prev) => ({
      ...prev,
      [field]: value,
  ...
  const handleGuardar = async () => {
    ...
    try {
      ...
      const payload: RealizarRevisionPrevioFirmaBanco = normalizeRealizarRevisionPrevioFirmaBanco(
        {
          ...form,
          id_realizar_revision_previo_firma_banco: Number(
            form.id_realizar_revision_previo_firma_banco ?? 0,
          ),
      ...
      if (response.status) {
        toast.current?.show({
          severity: "success",
          summary: "Éxito",
          detail: "Actividad Realizar revisión previo firma banco guardada correctamente",
          life: 3000,
        });

        const savedEntity = normalizeRealizarRevisionPrevioFirmaBanco(
      ...
      console.error("ERROR GUARDAR Realizar revisión previo firma banco", error);
  ...
  const handleAvanzar = async () => {
    ...
    try {
      ...
      console.error("ERROR AVANZAR Realizar revisión previo firma banco", error);
  ...
  return (
    <>
      ...
      <h2 className="text-2xl font-bold text-gray-900 mb-6">
        Realizar revisión previo firma banco
      </h2>
      ...
      <Accordion activeIndex={[0, 2]} multiple>
        ...
        <AccordionTab header="Realizar revisión previo firma banco">
