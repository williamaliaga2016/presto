...
import { useCorregirReparoEntregarCarpeta } from "../hooks/useCorregirReparoEntregarCarpeta";
import { useUpsertCorregirReparoEntregarCarpeta } from "../hooks/useUpsertCorregirReparoEntregarCarpeta";
import { useAvanzarCorregirReparoEntregarCarpeta } from "../hooks/useAvanzarCorregirReparoEntregarCarpeta";

const ACTIVITY_ID = "_ab12CD34EfGhIjKlMnOpQr";

const build_initial_state = (id_expediente: number): CorregirReparoEntregarCarpeta => ({
  id_corregir_reparo_entregar_carpeta: 0,
  id_expediente,
...
const normalize = (
  source: Partial<CorregirReparoEntregarCarpeta> | null | undefined,
  id_expediente_fallback: number,
): CorregirReparoEntregarCarpeta => ({
  id_corregir_reparo_entregar_carpeta: Number(source?.id_corregir_reparo_entregar_carpeta ?? 0),
...
export default function CorregirReparoEntregarCarpetaPage() {
  ...
  const [form, set_form] = useState<CorregirReparoEntregarCarpeta>(build_initial_state(id_expediente));
  ...
  const { data, isLoading } = useCorregirReparoEntregarCarpeta(id_expediente);
  const save_mutation = useUpsertCorregirReparoEntregarCarpeta();
  const avanzar_mutation = useAvanzarCorregirReparoEntregarCarpeta();
  ...
  useEffect(() => {
    ...
    if (data?.status && data.detail) {
      ...
      set_form(loaded);
      set_is_disabled(Number(data.detail.id_corregir_reparo_entregar_carpeta) > 0);
      set_can_advance(false);
  ...
  return (
    <>
      ...
      <h2 className="text-2xl font-bold text-gray-900 mb-6">Corregir Reparo Entregar Carpeta</h2>
...
interface ReparoTablaProps {
  form: CorregirReparoEntregarCarpeta;
  is_disabled: boolean;
