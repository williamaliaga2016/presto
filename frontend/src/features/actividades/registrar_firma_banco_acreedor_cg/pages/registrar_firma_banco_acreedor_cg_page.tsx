...
import { useRegistrarFirmaBancoAcreedorCG } from "../hooks/useRegistrarFirmaBancoAcreedorCG";
import { useUpsertRegistrarFirmaBancoAcreedorCG } from "../hooks/useUpsertRegistrarFirmaBancoAcreedorCG";
import { useAvanzarRegistrarFirmaBancoAcreedorCG } from "../hooks/useAvanzarRegistrarFirmaBancoAcreedorCG";

const ACTIVITY_ID = "_Kt9N_V7qAeaHv6_Q1mL4y";

const build_initial_state = (
  id_expediente: number,
): RegistrarFirmaBancoAcreedorCG => ({
  id_firma_banco_acreedor_cg: 0,
  id_expediente,
  observaciones: null,
  is_active: true,
  row_status: true,
...
const normalize = (
  source: Partial<RegistrarFirmaBancoAcreedorCG> | null | undefined,
  id_expediente_fallback: number,
): RegistrarFirmaBancoAcreedorCG => ({
  id_firma_banco_acreedor_cg: Number(
    source?.id_firma_banco_acreedor_cg ?? 0,
  ),
...
export default function RegistrarFirmaBancoAcreedorCGPage() {
  ...
  const [form, set_form] = useState<RegistrarFirmaBancoAcreedorCG>(
  ...
  const { data, isLoading } = useRegistrarFirmaBancoAcreedorCG(id_expediente);
  const save_mutation = useUpsertRegistrarFirmaBancoAcreedorCG();
  const avanzar_mutation = useAvanzarRegistrarFirmaBancoAcreedorCG();
  ...
  useEffect(() => {
    ...
    if (data?.status && data.detail) {
      set_form(normalize(data.detail, id_expediente));
      set_is_disabled(true);
      set_can_advance(false);
  ...
  const handle_guardar = async () => {
    ...
    try {
      ...
      if (response.status) {
        ...
        set_form(normalize(response.detail ?? payload, id_expediente));
        set_is_disabled(true);
  ...
  return (
    <>
      ...
      <Accordion multiple activeIndex={[2]}>
        ...
        <AccordionTab header="Registrar Firma Banco Acreedor CG">
          <Card className="w-full shadow-md card-presto-form mb-6">
            ...
            <div className="form-actions">
              ...
              <Button
                type="button"
                label={
                  avanzar_mutation.isPending ? "Avanzando..." : "Avanzar"
                }
