...
import GestionComercialSection from '../components/GestionComercialSection';
import type { RealizarGestionComercial } from '../models/realizar_gestion_comercial';
import { EMPTY_REALIZAR_GESTION_COMERCIAL } from '../models/realizar_gestion_comercial';
import { useRealizarGestionComercial } from '../hooks/useRealizarGestionComercial';
import { useUpsertRealizarGestionComercial } from '../hooks/useUpsertRealizarGestionComercial';
import { useAvanzarRealizarGestionComercial } from '../hooks/useAvanzarRealizarGestionComercial';

const ACTIVITY_ID = 'BBVA_ESCRITURACION_REALIZAR_GESTION_COMERCIAL';

export default function RealizarGestionComercialPage() {
  const toast = useRef<Toast>(null);
  const navigate = useNavigate();
  const { id_expediente: idExpedienteParam } = useParams();
  const id_expediente = Number(idExpedienteParam ?? 0);

  const [form, setForm] = useState<RealizarGestionComercial>(EMPTY_REALIZAR_GESTION_COMERCIAL(id_expediente));
  ...
  const { data, isLoading } = useRealizarGestionComercial(id_expediente);
  const saveMutation = useUpsertRealizarGestionComercial();
  const avanzarMutation = useAvanzarRealizarGestionComercial();

  useEffect(() => {
    if (currentExpedienteRef.current !== id_expediente) {
      currentExpedienteRef.current = id_expediente;
      hasHydratedRef.current = false;
      setForm(EMPTY_REALIZAR_GESTION_COMERCIAL(id_expediente));
  ...
  useEffect(() => {
    ...
    if (data?.status && data.detail) {
      ...
      const loaded: RealizarGestionComercial = { ...EMPTY_REALIZAR_GESTION_COMERCIAL(id_expediente), ...formulario, id_expediente };
  ...
  const updateField = <K extends keyof RealizarGestionComercial>(field: K, value: RealizarGestionComercial[K]) => {
  ...
  const handleGuardar = async () => {
    ...
    try {
      setIsBusy(true);
      const payload: RealizarGestionComercial = { ...form, id: Number(form.id ?? 0), id_expediente: Number(form.id_expediente || id_expediente || 0), id_actividad: ACTIVITY_ID, is_active: true, row_status: true };
      ...
      if (response.status) {
        ...
        setForm({ ...EMPTY_REALIZAR_GESTION_COMERCIAL(id_expediente), ...(response.detail ?? payload) });
  ...
  const ejecutarAvanzar = async () => {
    try {
      ...
      if (response.status) {
        const msg = form.cliente_desiste === 'SI' ? 'Caso cerrado. Fin Terminal.' : 'Actividad avanzada. Retorno al origen.';
        toast.current?.show({ severity: 'success', summary: 'Éxito', detail: msg, life: 2000 });
  ...
  const handleAvanzar = () => {
    if (!form.cliente_desiste) {
      toast.current?.show({ severity: 'warn', summary: 'Validación', detail: 'Campos obligatorios faltantes: ¿Cliente Desiste del Caso?', life: 5000 });
      return;
    }
  ...
  return (
    <>
      <Toast ref={toast} />
      <ConfirmDialog />
      <h2 className="text-2xl font-bold text-gray-900 mb-6">Realizar Gestión Comercial</h2>
      <Accordion activeIndex={[0, 2]} multiple>
        ...
        <AccordionTab header="Realizar Gestión Comercial">
          <Card className="w-full shadow-md card-presto-form mb-6">
            ...
            <div className="mb-6">
              <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wide mb-3">Información del Caso</h3>
              <DatosHeredadosSection datosHeredados={datosHeredados} />
            </div>

            <div className="border-t border-slate-200 my-4" />

            <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wide mb-3">Decisión Comercial</h3>
            <GestionComercialSection form={form} isDisabled={isDisabled} updateField={updateField} />
