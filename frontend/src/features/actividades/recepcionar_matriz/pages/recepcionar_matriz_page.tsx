...
import { useAvanzarRecepcionarMatriz } from "../hooks/useAvanzarRecepcionarMatriz";
import { useRecepcionarMatriz } from "../hooks/useRecepcionarMatriz";
import { useUpsertRecepcionarMatriz } from "../hooks/useUpsertRecepcionarMatriz";

const ACTIVITY_ID = "_Ep8L_T3kVebCf5_J9nD2q";

const buildInitialState = (id_expediente: number): RecepcionarMatriz => ({
  id_recepcionar_matriz: 0,
  id_expediente,
  observaciones: "",
  is_active: true,
  row_status: true,
...
const normalizeRecepcionarMatriz = (
  source: Partial<RecepcionarMatriz> | null | undefined,
  id_expediente_fallback: number,
): RecepcionarMatriz => {
  return {
    id_recepcionar_matriz: Number(source?.id_recepcionar_matriz ?? 0),
...
export default function RecepcionarMatrizPage() {
  ...
  const [form, setForm] = useState<RecepcionarMatriz>(
  ...
  const { data, isLoading } = useRecepcionarMatriz(id_expediente);
  const saveMutation = useUpsertRecepcionarMatriz();
  const avanzarMutation = useAvanzarRecepcionarMatriz();
  ...
  useEffect(() => {
    ...
    if (data?.status && data.detail) {
      const loadedEntity = normalizeRecepcionarMatriz(
        data.detail,
        id_expediente,
      );

      setForm(loadedEntity);
      setIsDisabled(Number(data.detail.id_recepcionar_matriz) > 0);
      setCanAdvance(false);
  ...
  const updateField = <K extends keyof RecepcionarMatriz>(
    field: K,
    value: RecepcionarMatriz[K],
  ) => {
    setForm((prev) => ({
      ...prev,
      [field]: value,
  ...
  const handleGuardar = async () => {
    ...
    try {
      ...
      const payload: RecepcionarMatriz = normalizeRecepcionarMatriz(
        {
          ...form,
          id_recepcionar_matriz: Number(
            form.id_recepcionar_matriz ?? 0,
          ),
      ...
      if (response.status) {
        toast.current?.show({
          severity: "success",
          summary: "Éxito",
          detail: "Actividad Recepcionar Matriz guardada correctamente",
          life: 3000,
        });

        const savedEntity = normalizeRecepcionarMatriz(
      ...
      console.error("ERROR GUARDAR Recepcionar Matriz", error);
  ...
  const handleAvanzar = async () => {
    ...
    try {
      ...
      console.error("ERROR AVANZAR Recepcionar Matriz", error);
  ...
  return (
    <>
      ...
      <Accordion activeIndex={[0, 2]} multiple>
        ...
        <AccordionTab header="Recepcionar Matriz">
