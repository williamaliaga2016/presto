...
import { useCorregirReparoFabrica } from "../hooks/useCorregirReparoFabrica";
import { useUpsertCorregirReparoFabrica } from "../hooks/useUpsertCorregirReparoFabrica";
import { useAvanzarCorregirReparoFabrica } from "../hooks/useAvanzarCorregirReparoFabrica";

const ACTIVITY_ID = "_ab12CD34EfGhIjKlMnOpQr";

const build_initial_state = (id_expediente: number): CorregirReparoFabrica => ({
  id_corregir_reparo_fabrica: 0,
  id_expediente,
...
const normalize = (
  source: Partial<CorregirReparoFabrica> | null | undefined,
  id_expediente_fallback: number,
): CorregirReparoFabrica => ({
  id_corregir_reparo_fabrica: Number(source?.id_corregir_reparo_fabrica ?? 0),
...
export default function CorregirReparoFabricaPage() {
  ...
  const [form, set_form] = useState<CorregirReparoFabrica>(build_initial_state(id_expediente));
  ...
  const { data, isLoading } = useCorregirReparoFabrica(id_expediente);
  const save_mutation = useUpsertCorregirReparoFabrica();
  const avanzar_mutation = useAvanzarCorregirReparoFabrica();
  ...
  useEffect(() => {
    ...
    if (data?.status && data.detail) {
      ...
      set_form(loaded);
      set_is_disabled(Number(data.detail.id_corregir_reparo_fabrica) > 0);
      set_can_advance(false);
