...
import { useCorregirControlEscritura } from "../hooks/useCorregirControlEscritura";
import { useUpsertCorregirControlEscritura } from "../hooks/useUpsertCorregirControlEscritura";
import { useAvanzarCorregirControlEscritura } from "../hooks/useAvanzarCorregirControlEscritura";

const ACTIVITY_ID = "";

const build_initial_state = (id_expediente: number): CorregirControlEscritura => ({
  id_corregir_control_escritura: 0,
  id_expediente,
...
const normalize = (
  source: Partial<CorregirControlEscritura> | null | undefined,
  id_expediente_fallback: number,
): CorregirControlEscritura => ({
  id_corregir_control_escritura: Number(source?.id_corregir_control_escritura ?? 0),
...
export default function CorregirControlEscrituraPage() {
  ...
  const [form, set_form] = useState<CorregirControlEscritura>(build_initial_state(id_expediente));
  ...
  const { data, isLoading } = useCorregirControlEscritura(id_expediente);
  const save_mutation = useUpsertCorregirControlEscritura();
  const avanzar_mutation = useAvanzarCorregirControlEscritura();
  ...
  useEffect(() => {
    ...
    if (data?.status && data.detail) {
      ...
      set_form(loaded);
      set_is_disabled(Number(data.detail.id_corregir_control_escritura) > 0);
      set_can_advance(false);
  ...
  const handle_guardar = async () => {
    ...
    try {
      ...
      console.error("ERROR GUARDAR CORREGIR CONTROL ESCRITURA", error);
      toast.current?.show({
        severity: "error",
        summary: "Error",
  ...
  const handle_avanzar = async () => {
    ...
    try {
      ...
      console.error("ERROR AVANZAR CORREGIR CONTROL ESCRITURA", error);
  ...
  return (
    <>
      ...
      <h2 className="text-2xl font-bold text-gray-900 mb-6">Corregir Control de Escritura</h2>
      ...
      <Accordion multiple activeIndex={[2]}>
        ...
        <AccordionTab header="Corregir Control de Escritura">
