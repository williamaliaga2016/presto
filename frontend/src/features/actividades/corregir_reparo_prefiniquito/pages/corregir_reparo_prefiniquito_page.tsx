...
import { useCorregirReparoPrefiniquito } from "../hooks/useCorregirReparoPrefiniquito";
import { useUpsertCorregirReparoPrefiniquito } from "../hooks/useUpsertCorregirReparoPrefiniquito";
import { useAvanzarCorregirReparoPrefiniquito } from "../hooks/useAvanzarCorregirReparoPrefiniquito";

const ACTIVITY_ID = "_Bp7Z_W1gQebYz2_G2dX9m";

const build_initial_state = (id_expediente: number): CorregirReparoPrefiniquito => ({
  id_corregir_reparo_prefiniquito: 0,
  id_expediente,
...
const normalize = (
  source: Partial<CorregirReparoPrefiniquito> | null | undefined,
  id_expediente_fallback: number,
): CorregirReparoPrefiniquito => ({
  id_corregir_reparo_prefiniquito: Number(source?.id_corregir_reparo_prefiniquito ?? 0),
  ...
  observaciones_origen: source?.observaciones_origen ?? "",
...
export default function CorregirReparoPrefiniquitoPage() {
  ...
  const [form, set_form] = useState<CorregirReparoPrefiniquito>(build_initial_state(id_expediente));
  ...
  const { data, isLoading } = useCorregirReparoPrefiniquito(id_expediente);
  const save_mutation = useUpsertCorregirReparoPrefiniquito();
  const avanzar_mutation = useAvanzarCorregirReparoPrefiniquito();
  ...
  useEffect(() => {
    ...
    if (data?.status && data.detail) {
      ...
      set_form(loaded);
      set_is_disabled(Number(data.detail.id_corregir_reparo_prefiniquito) > 0);
      set_can_advance(false);
  ...
  const handle_guardar = async () => {
    ...
    try {
      ...
      console.error("ERROR GUARDAR CORREGIR REPARO PREFINIQUITO", error);
      toast.current?.show({
        severity: "error",
        summary: "Error",
  ...
  const handle_avanzar = async () => {
    ...
    try {
      ...
      console.error("ERROR AVANZAR CORREGIR REPARO PREFINIQUITO", error);
  ...
  return (
    <>
      ...
      <h2 className="text-2xl font-bold text-gray-900 mb-6">Corregir Reparo Prefiniquito</h2>
      ...
      <Accordion multiple activeIndex={[2]}>
        ...
        <AccordionTab header="Corregir Reparo Prefiniquito">
...
interface ReparoTablaProps {
  form: CorregirReparoPrefiniquito;
  is_disabled: boolean;
        ...
        <table className="w-full border-collapse">
          ...
          <tbody>
            <tr className="bg-white">
              ...
              <td className="px-3 py-2 text-sm border border-gray-200">{form.observaciones_origen || "-"}</td>
