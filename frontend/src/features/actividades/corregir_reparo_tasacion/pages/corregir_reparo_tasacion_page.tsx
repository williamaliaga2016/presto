...
import { useCorregirReparoTasacion } from "../hooks/useCorregirReparoTasacion";
import { useUpsertCorregirReparoTasacion } from "../hooks/useUpsertCorregirReparoTasacion";
import { useAvanzarCorregirReparoTasacion } from "../hooks/useAvanzarCorregirReparoTasacion";

const ACTIVITY_ID = "_H9qBnV1tEeePw4_Y8mK2c";

const build_initial_state = (id_expediente: number): CorregirReparoTasacion => ({
  id_corregir_reparo_tasacion: 0,
  id_expediente,
...
const normalize = (
  source: Partial<CorregirReparoTasacion> | null | undefined,
  id_expediente_fallback: number,
): CorregirReparoTasacion => ({
  id_corregir_reparo_tasacion: Number(source?.id_corregir_reparo_tasacion ?? 0),
...
export default function CorregirReparoTasacionPage() {
  ...
  const [form, set_form] = useState<CorregirReparoTasacion>(build_initial_state(id_expediente));
  ...
  const { data, isLoading } = useCorregirReparoTasacion(id_expediente);
  const save_mutation = useUpsertCorregirReparoTasacion();
  const avanzar_mutation = useAvanzarCorregirReparoTasacion();
  ...
  useEffect(() => {
    ...
    if (data?.status && data.detail) {
      ...
      set_form(loaded);
      set_is_disabled(Number(data.detail.id_corregir_reparo_tasacion) > 0);
      set_can_advance(false);
  ...
  const handle_guardar = async () => {
    ...
    try {
      ...
      console.error("ERROR GUARDAR CORREGIR REPARO TASACION", error);
      toast.current?.show({
        severity: "error",
        summary: "Error",
  ...
  const handle_avanzar = async () => {
    ...
    try {
      ...
      console.error("ERROR AVANZAR CORREGIR REPARO TASACION", error);
  ...
  return (
    <>
      ...
      <h2 className="text-2xl font-bold text-gray-900 mb-6">Corregir Reparo Tasación</h2>
      ...
      <Accordion multiple activeIndex={[2]}>
        ...
        <AccordionTab header="Corregir Reparo Tasación">
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="flex items-center gap-2 h-11">
                  <Checkbox
                    ...
                    onChange={(e) => toggle_subsanar(!!e.checked)}
                    disabled={is_disabled}
                  />
                  <label htmlFor="is_subsanar" className="text-sm">
                    Marcar reparo como subsanado
                  </label>
                </div>
              </div>
...
interface ReparoTablaProps {
  form: CorregirReparoTasacion;
  is_disabled: boolean;
}

function ReparoTabla({ form, is_disabled }: ReparoTablaProps) {
  return (
    <div>
