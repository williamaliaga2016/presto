...
import { useAvanzarEntregarCarpeta } from "../hooks/useAvanzarEntregarCarpeta";
import { useEntregarCarpeta } from "../hooks/useEntregarCarpeta";
import { useUpsertEntregarCarpeta } from "../hooks/useUpsertEntregarCarpeta";

const ACTIVITY_ID = "_F5kTyM7wEeoJq8_Y2nV6p";

const buildInitialState = (id_expediente: number): EntregarCarpeta => ({
  id_entregar_carpeta: 0,
  id_expediente,
...
const normalizeEntregarCarpeta = (
  source: Partial<EntregarCarpeta> | null | undefined,
  id_expediente_fallback: number,
): EntregarCarpeta => {
  return {
    id_entregar_carpeta: Number(source?.id_entregar_carpeta ?? 0),
...
export default function EntregarCarpetaPage() {
  ...
  const [form, setForm] = useState<EntregarCarpeta>(
  ...
  const { data, isLoading } = useEntregarCarpeta(id_expediente);
  const saveMutation = useUpsertEntregarCarpeta();
  const avanzarMutation = useAvanzarEntregarCarpeta();
  ...
  useEffect(() => {
    ...
    if (data?.status && data.detail) {
      const loadedEntity = normalizeEntregarCarpeta(
        data.detail,
        id_expediente,
      );

      setForm(loadedEntity);
      setIsDisabled(Number(data.detail.id_entregar_carpeta) > 0);
      setCanAdvance(false);
  ...
  const updateField = <K extends keyof EntregarCarpeta>(
    field: K,
    value: EntregarCarpeta[K],
  ) => {
    setForm((prev) => ({
      ...prev,
      [field]: value,
  ...
  const handleGuardar = async () => {
    ...
    try {
      ...
      const payload: EntregarCarpeta = normalizeEntregarCarpeta(
        {
          ...form,
          id_entregar_carpeta: Number(
            form.id_entregar_carpeta ?? 0,
          ),
      ...
      if (response.status) {
        toast.current?.show({
          severity: "success",
          summary: "Éxito",
          detail: "Actividad entregar carpeta guardada correctamente",
          life: 3000,
        });

        const savedEntity = normalizeEntregarCarpeta(
