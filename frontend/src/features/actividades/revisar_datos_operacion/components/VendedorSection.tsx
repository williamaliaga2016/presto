...
import { EMPTY_CONTROLES_VENDEDOR, type CatalogoOption, type ControlesVendedor } from '../models/catalogo';
import {
  buildVendedorEmpty,
...
type Props = {
  value: RevisarDatosOperacionVendedor[];
  idExpediente: number;
  idRevisarDatosOperacion: number;
  disabled: boolean;
  controles?: ControlesVendedor;
  loadingControles?: boolean;
  onChange: (rows: RevisarDatosOperacionVendedor[]) => void;
...
const isJuridica = (row: RevisarDatosOperacionVendedor): boolean => {
...
const fullName = (row: RevisarDatosOperacionVendedor): string =>
...
type RequiredField = keyof RevisarDatosOperacionVendedor;

const getRequiredFields = (row: RevisarDatosOperacionVendedor): RequiredField[] => {
...
const getMissingField = (row: RevisarDatosOperacionVendedor): string | null => {
...
export default function VendedorSection({
  value,
  idExpediente,
  idRevisarDatosOperacion,
  disabled,
  controles = EMPTY_CONTROLES_VENDEDOR,
  ...
  const [selected, setSelected] = useState<RevisarDatosOperacionVendedor | null>(null);
  ...
  const tipoPersonaOptions = controles.tipo_vendedor ?? [];
  ...
  const siNoOptions = controles.si_no;

  const openCreate = () => {
    if (!canEdit) return;
    setSelected(buildVendedorEmpty(idExpediente, idRevisarDatosOperacion));
    setDialogMode('create');
    setDialogVisible(true);
  };

  const openView = (row: RevisarDatosOperacionVendedor) => {
    setSelected({ ...row });
    setDialogMode('view');
    setDialogVisible(true);
  };

  const openEdit = (row: RevisarDatosOperacionVendedor) => {
    if (!canEdit) return;
    setSelected({ ...row });
    setDialogMode('edit');
    setDialogVisible(true);
  ...
  const updateField = <K extends keyof RevisarDatosOperacionVendedor>(
    field: K,
    fieldValue: RevisarDatosOperacionVendedor[K],
  ) => {
    if (!selected) return;
  ...
  const saveRow = () => {
    ...
    const normalized: RevisarDatosOperacionVendedor = {
      ...selected,
    ...
    const selectedId = selected.id_revisar_datos_operacion_vendedor;

    if (dialogMode === 'create' || selectedId <= 0) {
      const localId = selectedId < 0 ? selectedId : -Date.now();
      onChange([...value, { ...normalized, id_revisar_datos_operacion_vendedor: localId }]);
      closeDialog();
      return;
    }
    ...
    onChange(
      value.map((item) =>
        item.id_revisar_datos_operacion_vendedor === selectedId ? normalized : item,
      ),
    );
    closeDialog();
  };

  const deleteRow = (row: RevisarDatosOperacionVendedor) => {
    const rowId = row.id_revisar_datos_operacion_vendedor;

    if (rowId <= 0) {
      onChange(value.filter((item) => item.id_revisar_datos_operacion_vendedor !== rowId));
      return;
    }

    onChange(
      value.map((item) =>
        item.id_revisar_datos_operacion_vendedor === rowId
  ...
  const confirmDelete = (row: RevisarDatosOperacionVendedor) => {
    if (!canEdit) return;
    confirmDialog({
  ...
  const actionsTemplate = (row: RevisarDatosOperacionVendedor) => (
  ...
  return (
    <Card title="Datos del Vendedor" className="shadow-sm border border-gray-200 rounded-xl">
      <ConfirmDialog />
      ...
      <Dialog
        header={`${dialogMode === 'view' ? 'Detalle' : dialogMode === 'edit' ? 'Editar' : 'Nuevo'} Vendedor`}
        ...
        {selected && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            ...
            <div className="flex flex-col gap-1">
              ...
              <Dropdown
                className="form-dropdown-presto w-full"
                value={selected.enviar_reparo ?? null}
                options={siNoOptions}
                optionLabel="description"
                optionValue="code"
                disabled={controlDisabled}
                placeholder="Seleccione"
                onChange={(e) => updateField('enviar_reparo', e.value)}
              />
            </div>
