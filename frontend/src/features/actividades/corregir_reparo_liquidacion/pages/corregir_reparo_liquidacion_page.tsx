...
import { useCorregirReparoLiquidacion } from "../hooks/useCorregirReparoLiquidacion";
import { useUpsertCorregirReparoLiquidacion } from "../hooks/useUpsertCorregirReparoLiquidacion";
import { useAvanzarCorregirReparoLiquidacion } from "../hooks/useAvanzarCorregirReparoLiquidacion";

const ACTIVITY_ID = "_Dr5B_Y7iSebAb9_J9fZ6p";

const build_initial_state = (id_expediente: number): CorregirReparoLiquidacion => ({
  id_corregir_reparo_liquidacion: 0,
  id_expediente,
...
const normalize = (
  source: Partial<CorregirReparoLiquidacion> | null | undefined,
  id_expediente_fallback: number,
): CorregirReparoLiquidacion => ({
  id_corregir_reparo_liquidacion: Number(source?.id_corregir_reparo_liquidacion ?? 0),
...
export default function CorregirReparoLiquidacionPage() {
  ...
  const [form, set_form] = useState<CorregirReparoLiquidacion>(build_initial_state(id_expediente));
  ...
  const { data, isLoading } = useCorregirReparoLiquidacion(id_expediente);
  const save_mutation = useUpsertCorregirReparoLiquidacion();
  const avanzar_mutation = useAvanzarCorregirReparoLiquidacion();
  ...
  useEffect(() => {
    ...
    if (data?.status && data.detail) {
      ...
      set_form(loaded);
      set_is_disabled(Number(data.detail.id_corregir_reparo_liquidacion) > 0);
      set_can_advance(false);
...
interface ReparoTablaProps {
  form: CorregirReparoLiquidacion;
  is_disabled: boolean;
