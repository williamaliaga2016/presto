...
import { useRegistrarFirmaComprador } from '../hooks/useRegistrarFirmaComprador';
import { useAvanzarRegistrarFirmaComprador } from '../hooks/useAvanzarRegistrarFirmaComprador';
import { useUpsertRegistrarFirmaComprador } from '../hooks/useUpsertRegistrarFirmaComprador';

const ACTIVITY_ID = '_Ak3L_P8qTebYv7_D2nH5x';
...
export default function RegistrarFirmaCompradorPage({
  ...
  const [isSaving, setIsSaving] = useState(false);
    ...
    const avanzarMutation = useAvanzarRegistrarFirmaComprador();
  const {
    data: firmaData,
    isLoading,
  } = useRegistrarFirmaComprador(idExpediente);
...
const upsertMutation = useUpsertRegistrarFirmaComprador();
...
const handleGuardar = async () => {
  ...
  try {
    ...
    const payload = {
      id_firma_comprador: firmaData?.detail?.id_firma_comprador ?? 0,
      ...
      firma_comprador_detalle: rows.map((r) => ({
        id_firma_comprador_detalle: r.id_firma_comprador_detalle ?? 0,
        id_firma_comprador: firmaData?.detail?.id_firma_comprador ?? 0,
        id_expediente: idExpediente,
    ...
    showToast('success', 'OK', 'Guardado correctamente');
  } catch (error) {
    const message =
  ...
  if (firmaData?.detail) {
    ...
    setRows(
      d.firma_comprador_detalle?.map((x) => ({
        id_firma_comprador_detalle: x.id_firma_comprador_detalle,
        ...
        fechaFirma: x.fecha_firma ? new Date(x.fecha_firma) : null,
      })) ?? []
    );
  }
}, [firmaData]);
  ...
  return (
    <div>
      ...
      <Accordion multiple activeIndex={[0, 1, 2]}>
        ...
        <AccordionTab header="Registrar Firma Comprador">
          <Card
              title="Registrar Firma Comprador"
