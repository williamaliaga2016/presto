...
import { useAvanzarCierreCopiasNotaria } from "../hooks/useAvanzarCierreCopiasNotaria";
import { useCierreCopiasNotaria } from "../hooks/useCierreCopiasNotaria";
import { useUpsertCierreCopiasNotaria } from "../hooks/useUpsertCierreCopiasNotaria";

const ACTIVITY_ID = "_Hz2B_Y6xTebUc5_N9fZ2u";

const buildInitialState = (id_expediente: number): CierreCopiasNotaria => ({
  id_cierre_copias_notaria: 0,
  id_expediente,
...
const normalizeCierreCopiasNotaria = (
  source: Partial<CierreCopiasNotaria> | null | undefined,
  id_expediente_fallback: number,
): CierreCopiasNotaria => {
  return {
    id_cierre_copias_notaria: Number(source?.id_cierre_copias_notaria ?? 0),
...
export default function CierreCopiasNotariaPage() {
  ...
  const [form, setForm] = useState<CierreCopiasNotaria>(
  ...
  const { data, isLoading } = useCierreCopiasNotaria(id_expediente);
  const saveMutation = useUpsertCierreCopiasNotaria();
  const avanzarMutation = useAvanzarCierreCopiasNotaria();
  ...
  useEffect(() => {
    ...
    if (data?.status && data.detail) {
      const loadedEntity = normalizeCierreCopiasNotaria(
        data.detail,
        id_expediente,
      );

      setForm(loadedEntity);
      setIsDisabled(Number(data.detail.id_cierre_copias_notaria) > 0);
      setCanAdvance(false);
  ...
  const updateField = <K extends keyof CierreCopiasNotaria>(
    field: K,
    value: CierreCopiasNotaria[K],
  ) => {
    setForm((prev) => ({
      ...prev,
      [field]: value,
  ...
  const handleGuardar = async () => {
    ...
    try {
      ...
      const payload: CierreCopiasNotaria = normalizeCierreCopiasNotaria(
        {
          ...form,
          id_cierre_copias_notaria: Number(
            form.id_cierre_copias_notaria ?? 0,
          ),
      ...
      if (response.status) {
        ...
        const savedEntity = normalizeCierreCopiasNotaria(
  ...
  const handleAvanzar = async () => {
    ...
    try {
      ...
      console.error("ERROR AVANZAR CIERRE COPIAS NOTARIA", error);
  ...
  return (
    <>
      ...
      <Accordion activeIndex={[0, 2]} multiple>
        ...
        <AccordionTab header="Cierre Copias Notaria">
          <Card className="w-full shadow-md card-presto-form mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              ...
              <div className="flex flex-col gap-2 md:col-span-3">
                ...
                <div className="bg-[#0B6EA8] text-white font-semibold px-4 py-2 text-sm uppercase rounded-t-lg">
