...
import { useGestionRectificatoriaEscrituraFirmada } from "../hooks/useGestionRectificatoriaEscrituraFirmada";
import { useUpsertGestionRectificatoriaEscrituraFirmada } from "../hooks/useUpsertGestionRectificatoriaEscrituraFirmada";
import { useAvanzarGestionRectificatoriaEscrituraFirmada } from "../hooks/useAvanzarGestionRectificatoriaEscrituraFirmada";
...
import { useControlesRectificatoriaEscrituraFirmada } from "../hooks/useControlesGestionRectificatoriaEscituraFirmada";

const ACTIVITY_ID = "gestion_rectificatoria_escritura_firmada";

const buildInitialState = (id_expediente: number): GestionRectificatoriaEscrituraFirmada => ({
  id_gestion_rectificatoria_escritura_firmada: 0,
  id_rectificatoria_firma_comprado_vendedor: 0,
  id_expediente,
  enviar_tipo_reparo: "",
...
const normalize = (
  source: Partial<GestionRectificatoriaEscrituraFirmada> | null | undefined,
  id_expediente_fallback: number,
): GestionRectificatoriaEscrituraFirmada => {
  const isExistingRecord = Number(source?.id_gestion_rectificatoria_escritura_firmada ?? 0) > 0;
  
  return {
    id_gestion_rectificatoria_escritura_firmada: Number(source?.id_gestion_rectificatoria_escritura_firmada ?? 0),
    id_rectificatoria_firma_comprado_vendedor: Number(source?.id_rectificatoria_firma_comprado_vendedor ?? source?.id_solicitud ?? 0),
...
export default function GestionRectificatoriaEscrituraFirmadaPage() {
  ...
  const [form, setForm] = useState<GestionRectificatoriaEscrituraFirmada>(buildInitialState(id_expediente));
  ...
  const { data: controlesResponse, isLoading: isLoadingControles } = useControlesRectificatoriaEscrituraFirmada(hasValidExpediente);
  const tipoReparoOptions = controlesResponse?.detail?.tipo_reparo ?? [];

  const { data, isLoading, refetch } = useGestionRectificatoriaEscrituraFirmada(id_expediente);
  const saveMutation = useUpsertGestionRectificatoriaEscrituraFirmada();
  const avanzarMutation = useAvanzarGestionRectificatoriaEscrituraFirmada();
  ...
  useEffect(() => {
    ...
    if (!hasValidExpediente) {
      ...
      setIsDisabled(true); // CORREGIDO: Siempre true
      setCanAdvance(false);
      hasHydratedRef.current = true;
      return;
    }
  ...
  const updateField = (field: keyof GestionRectificatoriaEscrituraFirmada, value: any) => {
  ...
  const handleGuardar = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      ...
      const payload = normalize({ 
        ...
        id_rectificatoria_firma_comprado_vendedor: form.id_solicitud,
      }, id_expediente);
      ...
      if (response.status) {
        toast.current?.show({
          severity: "success",
          summary: "Éxito",
          detail: response.message || "Gestión Rectificatoria Escritura Firmada guardada correctamente",
          life: 3000,
        });
  ...
  return (
    <div>
      <Toast ref={toast} />
      <h2 className="text-2xl font-bold text-gray-900 mb-6">Gestión Rectificatoria Escritura Firmada</h2>
      ...
      <Accordion multiple activeIndex={[0, 1, 2]}>
        ...
        <AccordionTab header="Gestión Rectificatoria Escritura Firmada" disabled={!hasValidExpediente}>
          {!hasValidExpediente ? (
            <Card>
