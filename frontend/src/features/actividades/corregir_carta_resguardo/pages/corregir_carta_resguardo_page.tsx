...
import { useCorregirCartaResguardo } from "../hooks/useCorregirCartaResguardo";
import { useUpsertCorregirCartaResguardo } from "../hooks/useUpsertCorregirCartaResguardo";
import { useAvanzarCorregirCartaResguardo } from "../hooks/useAvanzarCorregirCartaResguardo";

const ACTIVITY_ID = "_Gq7V_M2pXebEr8_M5wJ1c";

const build_initial_state = (id_expediente: number): CorregirCartaResguardo => ({
  id_corregir_carta_resguardo: 0,
  id_expediente,
...
const normalize = (
  source: Partial<CorregirCartaResguardo> | null | undefined,
  id_expediente_fallback: number,
): CorregirCartaResguardo => ({
  id_corregir_carta_resguardo: Number(source?.id_corregir_carta_resguardo ?? 0),
...
export default function CorregirCartaResguardoPage() {
  ...
  const [form, set_form] = useState<CorregirCartaResguardo>(build_initial_state(id_expediente));
  ...
  const { data, isLoading } = useCorregirCartaResguardo(id_expediente);
  const save_mutation = useUpsertCorregirCartaResguardo();
  const avanzar_mutation = useAvanzarCorregirCartaResguardo();
  ...
  useEffect(() => {
    ...
    if (data?.status && data.detail) {
      ...
      set_form(loaded);
      set_is_disabled(Number(data.detail.id_corregir_carta_resguardo) > 0);
      set_can_advance(false);
  ...
  const handle_guardar = async () => {
    ...
    try {
      ...
      console.error("ERROR GUARDAR CORREGIR CARTA RESGUARDO", error);
      toast.current?.show({
        severity: "error",
        summary: "Error",
  ...
  const handle_avanzar = async () => {
    ...
    try {
      ...
      console.error("ERROR AVANZAR CORREGIR CARTA RESGUARDO", error);
  ...
  return (
    <>
      ...
      <h2 className="text-2xl font-bold text-gray-900 mb-6">Corregir Carta de Resguardo</h2>
      ...
      <Accordion multiple activeIndex={[2]}>
        ...
        <AccordionTab header="Corregir Carta de Resguardo">
            ...
            <div className="flex flex-col gap-6">
              ...
              <div className="flex flex-col gap-1">
                ...
                <InputTextarea
                  ...
                  placeholder="Ingrese comentarios relevantes sobre la corrección de la Carta de Resguardo"
                />
              </div>
            </div>
