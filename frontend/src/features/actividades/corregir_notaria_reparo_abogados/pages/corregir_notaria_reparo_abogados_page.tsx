...
import { useCorregirNotariaReparoAbogados } from "../hooks/useCorregirNotariaReparoAbogados";
import { useUpsertCorregirNotariaReparoAbogados } from "../hooks/useUpsertCorregirNotariaReparoAbogados";
import { useAvanzarCorregirNotariaReparoAbogados } from "../hooks/useAvanzarCorregirNotariaReparoAbogados";

const ACTIVITY_ID = "_Mx5V_N1sCeaKq9_S2pF8w";

const build_initial_state = (id_expediente: number): CorregirNotariaReparoAbogados => ({
  id_corregir_notaria_reparo_abogados: 0,
  id_expediente,
...
const normalize = (
  source: Partial<CorregirNotariaReparoAbogados> | null | undefined,
  id_expediente_fallback: number,
): CorregirNotariaReparoAbogados => ({
  id_corregir_notaria_reparo_abogados: Number(source?.id_corregir_notaria_reparo_abogados ?? 0),
...
export default function CorregirNotariaReparoAbogadosPage() {
  ...
  const [form, set_form] = useState<CorregirNotariaReparoAbogados>(build_initial_state(id_expediente));
  ...
  const { data, isLoading } = useCorregirNotariaReparoAbogados(id_expediente);
  const save_mutation = useUpsertCorregirNotariaReparoAbogados();
  const avanzar_mutation = useAvanzarCorregirNotariaReparoAbogados();
  ...
  useEffect(() => {
    ...
    if (data?.status && data.detail) {
      ...
      set_form(loaded);
      set_is_disabled(Number(data.detail.id_corregir_notaria_reparo_abogados) > 0);
      set_can_advance(false);
  ...
  const handle_guardar = async () => {
    ...
    try {
      ...
      console.error("ERROR GUARDAR CORREGIR NOTARIA REPARO ABOGADOS", error);
      toast.current?.show({
        severity: "error",
        summary: "Error",
  ...
  const handle_avanzar = async () => {
    ...
    try {
      ...
      console.error("ERROR AVANZAR CORREGIR NOTARIA REPARO ABOGADOS", error);
  ...
  return (
    <>
      ...
      <h2 className="text-2xl font-bold text-gray-900 mb-6">Corregir Notaria Reparo de Abogaos</h2>
      ...
      <Accordion multiple activeIndex={[2]}>
        ...
        <AccordionTab header="Corregir Notaria Reparo de Abogaos">
...
interface ReparoTablaProps {
  form: CorregirNotariaReparoAbogados;
  is_disabled: boolean;
