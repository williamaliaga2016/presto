...
import { useAvanzarRevisarCopiasEscrituras } from "../hooks/useAvanzarRevisarCopiasEscrituras";
import { useRevisarCopiasEscrituras } from "../hooks/useRevisarCopiasEscrituras";
import { useUpsertRevisarCopiasEscrituras } from "../hooks/useUpsertRevisarCopiasEscrituras";
import { RadioButton } from "primereact/radiobutton";
import { Dropdown } from "primereact/dropdown";
import { useControlesRevisarCopiasEscrituras } from "../hooks/useControlesRevisarCopiasEscrituras";
import { EMPTY_CONTROLES_REVISAR_COPIAS_ESCRITURAS } from "../models/catalogo";

const ACTIVITY_ID = "_Fm4K_N9rWeaBh7_L2tP6z";

const buildInitialState = (id_expediente: number): RevisarCopiasEscrituras => ({
    id_revisar_copias_escrituras: 0,
    id_expediente,
    cbr: "",
...
const normalizeRevisarCopiasEscrituras = (
    source: Partial<RevisarCopiasEscrituras> | null | undefined,
    id_expediente_fallback: number,
): RevisarCopiasEscrituras => {
    return {
        id_revisar_copias_escrituras: Number(source?.id_revisar_copias_escrituras ?? 0),
...
export default function RevisarCopiasEscriturasPage() {
    ...
    const [form, setForm] = useState<RevisarCopiasEscrituras>(
    ...
    const { data, isLoading } = useRevisarCopiasEscrituras(id_expediente);
    const saveMutation = useUpsertRevisarCopiasEscrituras();
    const avanzarMutation = useAvanzarRevisarCopiasEscrituras();
    const controlesRevisarCopiasEscriturasQuery = useControlesRevisarCopiasEscrituras();
    
    const controlesRevisarCopiasEscrituras = controlesRevisarCopiasEscriturasQuery.data?.status
        ? controlesRevisarCopiasEscriturasQuery.data.detail ?? EMPTY_CONTROLES_REVISAR_COPIAS_ESCRITURAS
        : EMPTY_CONTROLES_REVISAR_COPIAS_ESCRITURAS;
    ...
    useEffect(() => {
        ...
        if (data?.status && data.detail) {
            const loadedEntity = normalizeRevisarCopiasEscrituras(
                data.detail,
                id_expediente,
            );
        ...
        if (data) {
            setForm((prev) =>
                normalizeRevisarCopiasEscrituras(
                    {
                        ...prev,
                        id_expediente,
                    },
    ...
    const updateField = <K extends keyof RevisarCopiasEscrituras>(
        field: K,
        value: RevisarCopiasEscrituras[K],
    ) => {
        setForm((prev) => ({
            ...prev,
            [field]: value,
    ...
    const handleGuardar = async () => {
        ...
        try {
            ...
            const payload: RevisarCopiasEscrituras = normalizeRevisarCopiasEscrituras(
                {
                    ...form,
                    id_revisar_copias_escrituras: Number(
                        form.id_revisar_copias_escrituras ?? 0,
                    ),
            ...
            if (response.status) {
                toast.current?.show({
                    severity: "success",
                    summary: "Éxito",
                    detail: "Actividad Revisar Copias Escritura guardada correctamente",
                    life: 3000,
                });

                const savedEntity = normalizeRevisarCopiasEscrituras(
            ...
            console.error("ERROR GUARDAR Revisar Copias Escritura", error);
    ...
    const handleAvanzar = async () => {
        ...
        try {
            ...
            console.error("ERROR AVANZAR Revisar Copias Escritura", error);
    ...
    return (
        <>
            ...
            <Accordion activeIndex={[0, 2]} multiple>
                ...
                <AccordionTab header="Revisar Copias Escritura">
                    <Card className="w-full shadow-md card-presto-form mb-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                            ...
                            <div className="flex flex-col gap-1">
                                <label className="font-semibold text-sm">Seleccionar CBR</label>
                                <Dropdown
                                    value={form?.cbr ?? null}
                                    options={controlesRevisarCopiasEscrituras.comuna}
                                    optionLabel="description"
                                    optionValue="code"
                                    disabled={isDisabled}
                                    className="form-dropdown-presto w-full"
                                    placeholder="Seleccione"
                                    onChange={(e) => updateField('cbr', e.value)}
                                    showClear
                                />
                            </div>
