...
import { useGenerarPreFiniquito } from "../hooks/useGenerarPreFiniquito";
import { useUpsertGenerarPreFiniquito } from "../hooks/useUpsertGenerarPreFiniquito";
import { useAvanzarGenerarPreFiniquito } from "../hooks/useAvanzarGenerarPreFiniquito";
...
const buildInitialState = (id_expediente: number): GenerarPreFiniquito => ({
  id_generar_prefiniquito: 0,
  id_expediente,
  observaciones: "",
  is_active: true,
  row_status: true,
...
const normalizeGenerarPreFiniquito = (
  source: Partial<GenerarPreFiniquito> | null | undefined,
  id_expediente_fallback: number,
): GenerarPreFiniquito => {
  return {
    id_generar_prefiniquito: Number(source?.id_generar_prefiniquito ?? 0),
...
export default function GenerarPreFiniquitoPage() {
  const toast = useRef<Toast>(null);
  const navigate = useNavigate();
  const { id_expediente: idExpedienteParam } = useParams();
  const id_expediente = Number(idExpedienteParam ?? 0);

  const [form, setForm] = useState<GenerarPreFiniquito>(buildInitialState(id_expediente));
  ...
  const { data, isLoading } = useGenerarPreFiniquito(id_expediente);
  const saveMutation = useUpsertGenerarPreFiniquito();
  const avanzarMutation = useAvanzarGenerarPreFiniquito();
  ...
  useEffect(() => {
    ...
    if (data?.status && data.detail) {
      setForm(normalizeGenerarPreFiniquito(data.detail, id_expediente));
      setIsDisabled(Number(data.detail.id_generar_prefiniquito) > 0);   // tiene datos â†’ bloquear hasta que editen
      setCanAdvance(false);
    ...
    if (data) {
      setForm((prev) => normalizeGenerarPreFiniquito({ ...prev, id_expediente }, id_expediente));
  ...
  const handleGuardar = async () => {
    ...
    try {
      setIsBusy(true);
      const payload = normalizeGenerarPreFiniquito(
      ...
      if (response.status) {
        ...
        setForm(normalizeGenerarPreFiniquito(response.detail ?? payload, payload.id_expediente));
        setIsDisabled(true);
        setCanAdvance(true);
        hasHydratedRef.current = true;
      } else {
