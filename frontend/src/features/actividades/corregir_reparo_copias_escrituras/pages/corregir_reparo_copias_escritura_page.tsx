...
import { useCorregirReparoCopiasEscrituras } from "../hooks/useCorregirReparoCopiasEscrituras";
import { useUpsertCorregirReparoCopiasEscrituras } from "../hooks/useUpsertCorregirReparoCopiasEscrituras";
import { useAvanzarCorregirReparoCopiasEscrituras } from "../hooks/useAvanzarCorregirReparoCopiasEscrituras";

const ACTIVITY_ID = "_Qc6N_R2vFeaNt8_V3sK7z";

const build_initial_state = (id_expediente: number): CorregirReparoCopiasEscrituras => ({
  id_corregir_reparo_copias_escrituras: 0,
  id_expediente,
...
const normalize = (
  source: Partial<CorregirReparoCopiasEscrituras> | null | undefined,
  id_expediente_fallback: number,
): CorregirReparoCopiasEscrituras => ({
  id_corregir_reparo_copias_escrituras: Number(source?.id_corregir_reparo_copias_escrituras ?? 0),
...
export default function CorregirReparoCopiasEscriturasPage() {
  ...
  const [form, set_form] = useState<CorregirReparoCopiasEscrituras>(
  ...
  const { data, isLoading } = useCorregirReparoCopiasEscrituras(id_expediente);
  const save_mutation = useUpsertCorregirReparoCopiasEscrituras();
  const avanzar_mutation = useAvanzarCorregirReparoCopiasEscrituras();
  ...
  useEffect(() => {
    ...
    if (data?.status && data.detail) {
      ...
      set_form(loaded);
      set_is_disabled(Number(data.detail.id_corregir_reparo_copias_escrituras) > 0);
      set_can_advance(false);
  ...
  const validate_form = (): string => {
    if (!form.observaciones || !form.observaciones.trim()) {
      return "Debe ingresar las Observaciones de Subsanación.";
    }
  ...
  const handle_guardar = async () => {
    ...
    try {
      ...
      console.error("ERROR GUARDAR CORREGIR REPARO COPIAS ESCRITURAS", error);
  ...
  const handle_avanzar = async () => {
    ...
    if (!form.is_subsanar) {
      const msg = "Debe marcar el reparo como subsanado antes de avanzar.";
    ...
    try {
      ...
      console.error("ERROR AVANZAR CORREGIR REPARO COPIAS ESCRITURAS", error);
  ...
  return (
    <>
      ...
      <h2 className="text-2xl font-bold text-gray-900 mb-6">
        Corregir Reparo Copias de Escrituras
      </h2>
      ...
      <Accordion multiple activeIndex={[2]}>
        ...
        <AccordionTab header="Corregir Reparo Copias de Escrituras">
          <Card className="w-full shadow-md card-presto-form mb-6">
            ...
            <div className="flex flex-col gap-6">
              ...
              <div className="flex flex-col gap-1">
                <label className="font-semibold text-sm">Observaciones de Subsanación *</label>
                <InputTextarea
                  ...
                  placeholder="Ingrese las observaciones de subsanación"
                />
              </div>
            </div>
...
interface ReparoTablaProps {
  form: CorregirReparoCopiasEscrituras;
  is_disabled: boolean;
...
function ReparoTabla({
  ...
  const has_reparo =
    form.id_corregir_reparo_copias_escrituras > 0 ||
    !!form.solicitante ||
