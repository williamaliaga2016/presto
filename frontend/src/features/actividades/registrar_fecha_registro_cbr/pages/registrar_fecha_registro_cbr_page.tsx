...
import { useAvanzarRegistrarFechaRegistroCbr } from "../hooks/useAvanzarRegistrarFechaRegistroCbr";
import { useRegistrarFechaRegistroCbr } from "../hooks/useRegistrarFechaRegistroCbr";
import { useUpsertRegistrarFechaRegistroCbr } from "../hooks/useUpsertRegistrarFechaRegistroCbr";

const ACTIVITY_ID = "_Vh5T_Q1aKeaSy8_A9xR6e";

const buildInitialState = (id_expediente: number): RegistrarFechaRegistroCbr => ({
  id_registrar_fecha_registro_cbr: 0,
  id_expediente,
  fecha_registro_cbr: new Date().toISOString(),
  observaciones: "",
  is_active: true,
  row_status: true,
  created_by: 0,
...
const normalizeRegistrarFechaRegistroCbr = (
  source: Partial<RegistrarFechaRegistroCbr> | null | undefined,
  id_expediente_fallback: number,
): RegistrarFechaRegistroCbr => {
  return {
    id_registrar_fecha_registro_cbr: Number(source?.id_registrar_fecha_registro_cbr ?? 0),
    id_expediente: Number(source?.id_expediente ?? id_expediente_fallback ?? 0),
    fecha_registro_cbr: source?.fecha_registro_cbr ?? new Date().toISOString(),
...
export default function RegistrarFechaRegistroCbrPage() {
  ...
  const [form, setForm] = useState<RegistrarFechaRegistroCbr>(
  ...
  const { data, isLoading } = useRegistrarFechaRegistroCbr(id_expediente);
  const saveMutation = useUpsertRegistrarFechaRegistroCbr();
  const avanzarMutation = useAvanzarRegistrarFechaRegistroCbr();
  ...
  useEffect(() => {
    ...
    if (data?.status && data.detail) {
      const loadedEntity = normalizeRegistrarFechaRegistroCbr(
        data.detail,
        id_expediente,
      );

      setForm(loadedEntity);
      setIsDisabled(Number(data.detail.id_registrar_fecha_registro_cbr) > 0);
      setCanAdvance(false);
    ...
    if (data) {
      setForm((prev) =>
        normalizeRegistrarFechaRegistroCbr(
          {
            ...prev,
            id_expediente,
          },
  ...
  const updateField = <K extends keyof RegistrarFechaRegistroCbr>(
    field: K,
    value: RegistrarFechaRegistroCbr[K],
  ) => {
    setForm((prev) => ({
      ...prev,
      [field]: value,
  ...
  const validateForm = () => {
    ...
    if (!form.fecha_registro_cbr) {
      return "La fecha de registro CBR es obligatoria.";
    }
  ...
  const handleGuardar = async () => {
    ...
    try {
      ...
      const payload: RegistrarFechaRegistroCbr = normalizeRegistrarFechaRegistroCbr(
        {
          ...form,
          id_registrar_fecha_registro_cbr: Number(
            form.id_registrar_fecha_registro_cbr ?? 0,
          ),
      ...
      if (response.status) {
        toast.current?.show({
          severity: "success",
          summary: "Éxito",
          detail: "Actividad Registrar Fecha Registro CBR guardada correctamente",
          life: 3000,
        });

        const savedEntity = normalizeRegistrarFechaRegistroCbr(
      ...
      console.error("ERROR GUARDAR Registrar Fecha Registro CBR", error);
  ...
  const handleAvanzar = async () => {
    ...
    try {
      ...
      console.error("ERROR AVANZAR Registrar Fecha Registro CBR", error);
  ...
  const fecha_as_date = form.fecha_registro_cbr
    ? new Date(form.fecha_registro_cbr)
    : new Date();
  ...
  return (
    <>
      ...
      <Accordion activeIndex={[0, 2]} multiple>
        ...
        <AccordionTab header="Registrar Fecha Registro CBR">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              ...
              <div className="flex flex-col gap-1">
                <label className="font-semibold text-sm">
                  Fecha Registro CBR <span className="text-red-500">*</span>
                </label>
                <Calendar
                  value={fecha_as_date}
                  onChange={(e) => {
