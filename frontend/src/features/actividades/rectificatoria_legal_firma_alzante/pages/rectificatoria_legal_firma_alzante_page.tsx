...
import { useRectificatoriaLegalFirmaAlzante } from "../hooks/useRectificatoriaLegalFirmaAlzante";
import { useUpsertRectificatoriaLegalFirmaAlzante } from "../hooks/useUpsertRectificatoriaLegalFirmaAlzante";
import { useAvanzarRectificatoriaLegalFirmaAlzante } from "../hooks/useAvanzarRectificatoriaLegalFirmaAlzante";

const ACTIVITY_ID = "";

const build_initial_state = (id_expediente: number): RectificatoriaLegalFirmaAlzante => ({
  id_rectificatoria_legal_firma_alzante: 0,
  id_expediente,
  id_usuario_solicitante: 0,
  fecha_firma_alzante: null,
  observaciones: null,
...
const normalize = (
  source: Partial<RectificatoriaLegalFirmaAlzante> | null | undefined,
  id_expediente_fallback: number,
): RectificatoriaLegalFirmaAlzante => ({
  id_rectificatoria_legal_firma_alzante: Number(source?.id_rectificatoria_legal_firma_alzante ?? 0),
  ...
  fecha_firma_alzante: source?.fecha_firma_alzante ?? null,
...
export default function RectificatoriaLegalFirmaAlzantePage() {
  ...
  const [form, set_form] = useState<RectificatoriaLegalFirmaAlzante>(build_initial_state(id_expediente));
  ...
  const { data, isLoading } = useRectificatoriaLegalFirmaAlzante(id_expediente);
  const save_mutation = useUpsertRectificatoriaLegalFirmaAlzante();
  const avanzar_mutation = useAvanzarRectificatoriaLegalFirmaAlzante();
  ...
  useEffect(() => {
    ...
    if (data?.status && data.detail) {
      ...
      const is_new = loaded.id_rectificatoria_legal_firma_alzante === 0;
      set_form(loaded);
      set_is_disabled(!is_new);
      set_can_advance(!is_new && !!loaded.fecha_firma_alzante);
      has_hydrated.current = true;
    }
  ...
  const validate_form = (): string => {
    if (!form.fecha_firma_alzante) {
      return "La fecha de firma del alzante es obligatoria.";
    }
  ...
  const handle_guardar = async () => {
    ...
    try {
      ...
      if (response.status) {
        ...
        set_form(saved);
        set_is_disabled(true);
        set_can_advance(!!saved.fecha_firma_alzante);
        has_hydrated.current = true;
      } else {
        toast.current?.show({
          severity: "warn",
      ...
      console.error("ERROR GUARDAR RECTIFICATORIA LEGAL FIRMA ALZANTE", error);
      toast.current?.show({
        severity: "error",
        summary: "Error",
  ...
  const handle_avanzar = async () => {
    ...
    try {
      ...
      console.error("ERROR AVANZAR RECTIFICATORIA LEGAL FIRMA ALZANTE", error);
  ...
  const fecha_as_date = form.fecha_firma_alzante ? new Date(form.fecha_firma_alzante) : null;
  ...
  return (
    <>
      ...
      <h2 className="text-2xl font-bold text-gray-900 mb-6">
        Rectificatoria Legal - Firma Alzante
      </h2>
      ...
      <Accordion multiple activeIndex={[2]}>
        ...
        <AccordionTab header="Rectificatoria Legal - Firma Alzante">
                      ...
                      <td className="border border-gray-300 px-3 py-2 align-middle">
                        <Calendar
                          ...
                          onChange={(e) => {
                            ...
                            set_form((prev) => ({
                              ...prev,
                              fecha_firma_alzante: val ? val.toISOString() : null,
                            }));
                          }}
                          dateFormat="dd/mm/yy"
                          showIcon
