...
import { useVerificarReparoIngresarDatosOperacion } from "../hooks/useVerificarReparoIngresarDatosOperacion";
import { useUpsertVerificarReparoIngresarDatosOperacion } from "../hooks/useUpsertVerificarReparoIngresarDatosOperacion";
import { useAvanzarVerificarReparoIngresarDatosOperacion } from "../hooks/useAvanzarVerificarReparoIngresarDatosOperacion";

const ACTIVITY_ID = "_V8mQcK3xEejBd7_X9pL5s";

const build_initial_state = (id_expediente: number): VerificarReparoDatosOperacion => ({
  id_verificar_reparo_datos_operacion: 0,
  id_expediente,
...
const normalize = (
  source: Partial<VerificarReparoDatosOperacion> | null | undefined,
  id_expediente_fallback: number,
): VerificarReparoDatosOperacion => ({
  id_verificar_reparo_datos_operacion: Number(source?.id_verificar_reparo_datos_operacion ?? 0),
...
export default function VerificarReparoIngresarDatosOperacionPage() {
  ...
  const [form, set_form] = useState<VerificarReparoDatosOperacion>(build_initial_state(id_expediente));
  ...
  const { data, isLoading } = useVerificarReparoIngresarDatosOperacion(id_expediente);
  const save_mutation = useUpsertVerificarReparoIngresarDatosOperacion();
  const avanzar_mutation = useAvanzarVerificarReparoIngresarDatosOperacion();
  ...
  useEffect(() => {
    ...
    if (data?.status && data.detail) {
      ...
      set_form(loaded);
      set_is_disabled(Number(data.detail.id_verificar_reparo_datos_operacion) > 0);
      set_can_advance(false);
  ...
  const handle_guardar = async () => {
    ...
    try {
      ...
      console.error("ERROR GUARDAR VERIFICAR REPARO INGRESAR DATOS OPERACION", error);
      toast.current?.show({
        severity: "error",
        summary: "Error",
  ...
  const handle_avanzar = async () => {
    ...
    try {
      ...
      console.error("ERROR AVANZAR VERIFICAR REPARO INGRESAR DATOS OPERACION", error);
  ...
  return (
    <>
      ...
      <h2 className="text-2xl font-bold text-gray-900 mb-6">Verificar Reparo Ingresar Datos Operación</h2>
      ...
      <Accordion multiple activeIndex={[2]}>
        ...
        <AccordionTab header="Verificar Reparo Ingresar Datos Operación">
...
interface ReparoTablaProps {
  form: VerificarReparoDatosOperacion;
  is_disabled: boolean;
