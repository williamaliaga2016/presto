...
import type { DatosOperacionFiadorGarante } from '../models/datos_operacion';
import { buildFiadorGaranteEmpty } from '../models/datos_operacion';
import {
...
type Props = {
  value: DatosOperacionFiadorGarante[];
  idExpediente: number;
  ...
  onChange: (rows: DatosOperacionFiadorGarante[]) => void;
...
const getName = (row: DatosOperacionFiadorGarante) =>
...
const requiredFiadorFields: { field: keyof DatosOperacionFiadorGarante; label: string }[] = [
  { field: 'rut', label: 'RUT' },
...
const getMissingFiadorField = (row: DatosOperacionFiadorGarante): string | null => {
...
export default function FiadorGaranteSection({
  ...
  const [selected, setSelected] = useState<DatosOperacionFiadorGarante | null>(null);
  ...
  const openCreate = () => {
    ...
    setSelected(buildFiadorGaranteEmpty(idExpediente, idDatosOperacion));
    setDialogMode('create');
    setDialogVisible(true);
  };

  const openView = (row: DatosOperacionFiadorGarante) => {
    setSelected({ ...row });
    setDialogMode('view');
    setDialogVisible(true);
  };

  const openEdit = (row: DatosOperacionFiadorGarante) => {
    if (!canEditActions) return;
    setSelected({ ...row });
    setDialogMode('edit');
    setDialogVisible(true);
  ...
  const updateField = <K extends keyof DatosOperacionFiadorGarante>(
    field: K,
    fieldValue: DatosOperacionFiadorGarante[K],
  ) => {
    if (!selected) return;
  ...
  const saveRow = () => {
    ...
    const normalized: DatosOperacionFiadorGarante = {
      ...selected,
    ...
    if (dialogMode === 'create' || normalized.id_datos_operacion_fiador_garante <= 0) {
      onChange([
        ...value,
        {
          ...normalized,
          id_datos_operacion_fiador_garante:
            normalized.id_datos_operacion_fiador_garante < 0
              ? normalized.id_datos_operacion_fiador_garante
              : -Date.now(),
        },
      ]);
      closeDialog();
    ...
    onChange(
      value.map((item) =>
        item.id_datos_operacion_fiador_garante === normalized.id_datos_operacion_fiador_garante
          ? normalized
          : item,
      ),
    );
  ...
  const deleteRow = (row: DatosOperacionFiadorGarante) => {
    const id = row.id_datos_operacion_fiador_garante;

    if (id <= 0) {
      onChange(value.filter((item) => item.id_datos_operacion_fiador_garante !== id));
      return;
    }

    onChange(
      value.map((item) =>
        item.id_datos_operacion_fiador_garante === id
  ...
  const confirmDelete = (row: DatosOperacionFiadorGarante) => {
    if (!canEditActions) return;
  ...
  const actionsTemplate = (row: DatosOperacionFiadorGarante) => (
  ...
  return (
    <Card title="Datos del Fiador / Garante" className="shadow-sm border border-gray-200 rounded-xl">
      ...
      >
        ...
        <Column key={`acciones-fiador-${canEditActions ? 'edit' : 'view'}`} header="Acciones" body={actionsTemplate} style={{ width: '11rem', textAlign: 'center' }} />
      </DataTable>
