...
import { useCorregirReparoVisado } from "../hooks/useCorregirReparoVisado";
import { useUpsertCorregirReparoVisado } from "../hooks/useUpsertCorregirReparoVisado";
import { useAvanzarCorregirReparoVisado } from "../hooks/useAvanzarCorregirReparoVisado";

const ACTIVITY_ID = "_S4vBnL2xEepWu3_X5qK1d";

const build_initial_state = (id_expediente: number): CorregirReparoVisado => ({
  id_corregir_reparo_visado: 0,
  id_expediente,
...
const normalize = (
  source: Partial<CorregirReparoVisado> | null | undefined,
  id_expediente_fallback: number,
): CorregirReparoVisado => ({
  id_corregir_reparo_visado: Number(source?.id_corregir_reparo_visado ?? 0),
...
export default function CorregirReparoVisadoPage() {
  ...
  const [form, set_form] = useState<CorregirReparoVisado>(
  ...
  const { data, isLoading } = useCorregirReparoVisado(id_expediente);
  const save_mutation = useUpsertCorregirReparoVisado();
  const avanzar_mutation = useAvanzarCorregirReparoVisado();
  ...
  useEffect(() => {
    ...
    if (data?.status && data.detail) {
      ...
      set_form(loaded);
      set_is_disabled(Number(data.detail.id_corregir_reparo_visado) > 0);
      set_can_advance(false);
  ...
  const handle_guardar = async () => {
    ...
    try {
      ...
      console.error("ERROR GUARDAR CORREGIR REPARO VISADO", error);
  ...
  const handle_avanzar = async () => {
    ...
    try {
      ...
      console.error("ERROR AVANZAR CORREGIR REPARO VISADO", error);
  ...
  return (
    <>
      ...
      <Accordion multiple activeIndex={[2]}>
        ...
        <AccordionTab header="Corregir Reparo Visado">
