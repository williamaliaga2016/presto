...
import { useCorregirReparoGenerarMemoEscritura } from "../hooks/useCorregirReparoGenerarMemoEscritura";
import { useUpsertCorregirReparoGenerarMemoEscritura } from "../hooks/useUpsertCorregirReparoGenerarMemoEscritura";
import { useAvanzarCorregirReparoGenerarMemoEscritura } from "../hooks/useAvanzarCorregirReparoGenerarMemoEscritura";

const ACTIVITY_ID = "_N9xFdC1vEenZh4_Z8rL2m";

const build_initial_state = (id_expediente: number): CorregirReparoGenerarMemoEscritura => ({
  id_corregir_reparo_generar_memo_escritura: 0,
  id_expediente,
...
const normalize = (
  source: Partial<CorregirReparoGenerarMemoEscritura> | null | undefined,
  id_expediente_fallback: number,
): CorregirReparoGenerarMemoEscritura => ({
  id_corregir_reparo_generar_memo_escritura: Number(
    source?.id_corregir_reparo_generar_memo_escritura ?? 0,
  ),
...
export default function CorregirReparoGenerarMemoEscrituraPage() {
  ...
  const [form, set_form] = useState<CorregirReparoGenerarMemoEscritura>(
  ...
  const { data, isLoading } = useCorregirReparoGenerarMemoEscritura(id_expediente);
  const save_mutation = useUpsertCorregirReparoGenerarMemoEscritura();
  const avanzar_mutation = useAvanzarCorregirReparoGenerarMemoEscritura();
  ...
  useEffect(() => {
    ...
    if (data?.status && data.detail) {
      ...
      set_form(loaded);
      set_is_disabled(Number(data.detail.id_corregir_reparo_generar_memo_escritura) > 0);
      set_can_advance(false);
  ...
  const handle_guardar = async () => {
    ...
    try {
      ...
      console.error("ERROR GUARDAR CORREGIR REPARO GENERAR MEMO ESCRITURA", error);
  ...
  const handle_avanzar = async () => {
    ...
    try {
      ...
      console.error("ERROR AVANZAR CORREGIR REPARO GENERAR MEMO ESCRITURA", error);
  ...
  return (
    <>
      ...
      <h2 className="text-2xl font-bold text-gray-900 mb-6">
        Corregir Reparo Generar Memo Escritura
      </h2>
      ...
      <Accordion multiple activeIndex={[2]}>
        ...
        <AccordionTab header="Corregir Reparo Generar Memo Escritura">
...
interface ReparoTablaProps {
  form: CorregirReparoGenerarMemoEscritura;
  is_disabled: boolean;
