...
import { useRevisarInscripcionCbr } from "../hooks/useRevisarInscripcionCbr";
import { useUpsertRevisarInscripcionCbr } from "../hooks/useUpsertRevisarInscripcionCbr";
import { useAvanzarRevisarInscripcionCbr } from "../hooks/useAvanzarRevisarInscripcionCbr";
 ...
 */
const ACTIVITY_ID = "_Wj8U_R4bLebTt2_B3yS1f";

const buildInitialState = (id_expediente: number): RevisarInscripcionCbr => ({
  id_revisar_inscripcion_cbr: 0,
  id_expediente,
...
const normalizeRevisarInscripcionCbr = (
  source: Partial<RevisarInscripcionCbr> | null | undefined,
  id_expediente_fallback: number,
): RevisarInscripcionCbr => {
  return {
    id_revisar_inscripcion_cbr: Number(source?.id_revisar_inscripcion_cbr ?? 0),
...
export default function RevisarInscripcionCbrPage() {
  ...
  const [form, setForm] = useState<RevisarInscripcionCbr>(
  ...
  const { data, isLoading } = useRevisarInscripcionCbr(id_expediente);
  const saveMutation = useUpsertRevisarInscripcionCbr();
  const avanzarMutation = useAvanzarRevisarInscripcionCbr();
  ...
  useEffect(() => {
    ...
    if (data?.status && data.detail) {
      const loadedEntity = normalizeRevisarInscripcionCbr(
        data.detail,
        id_expediente,
      );

      setForm(loadedEntity);
      setIsDisabled(Number(data.detail.id_revisar_inscripcion_cbr) > 0);
      setCanAdvance(false);
    ...
    if (data) {
      setForm((prev) =>
        normalizeRevisarInscripcionCbr(
          {
            ...prev,
            id_expediente,
          },
  ...
  const updateField = <K extends keyof RevisarInscripcionCbr>(
    field: K,
    value: RevisarInscripcionCbr[K],
  ) => {
    setForm((prev) => ({
      ...prev,
      [field]: value,
  ...
  const handleGuardar = async () => {
    ...
    try {
      ...
      const payload: RevisarInscripcionCbr = normalizeRevisarInscripcionCbr(
        {
          ...form,
          id_revisar_inscripcion_cbr: Number(
            form.id_revisar_inscripcion_cbr ?? 0,
          ),
      ...
      if (response.status) {
        ...
        const savedEntity = normalizeRevisarInscripcionCbr(
