...
import { useRectificatoriaLegalCierreCopias } from "../hooks/useRectificatoriaLegalCierreCopias";
import { useUpsertRectificatoriaLegalCierreCopias } from "../hooks/useUpsertRectificatoriaLegalCierreCopias";
import { useAvanzarRectificatoriaLegalCierreCopias } from "../hooks/useAvanzarRectificatoriaLegalCierreCopias";

const ACTIVITY_ID = "";

const build_initial_state = (id_expediente: number): RectificatoriaLegalCierreCopias => ({
  id_rectificatoria_legal_cierre_copias: 0,
  id_expediente,
...
const normalize = (
  source: Partial<RectificatoriaLegalCierreCopias> | null | undefined,
  id_expediente_fallback: number,
): RectificatoriaLegalCierreCopias => ({
  id_rectificatoria_legal_cierre_copias: Number(source?.id_rectificatoria_legal_cierre_copias ?? 0),
...
export default function RectificatoriaLegalCierreCopiasPage() {
  ...
  const [form, set_form] = useState<RectificatoriaLegalCierreCopias>(build_initial_state(id_expediente));
  ...
  const { data, isLoading } = useRectificatoriaLegalCierreCopias(id_expediente);
  const save_mutation = useUpsertRectificatoriaLegalCierreCopias();
  const avanzar_mutation = useAvanzarRectificatoriaLegalCierreCopias();
  ...
  useEffect(() => {
    ...
    if (data?.status && data.detail) {
      ...
      const is_new = loaded.id_rectificatoria_legal_cierre_copias === 0;
      set_form(loaded);
  ...
  const handle_guardar = async () => {
    ...
    try {
      ...
      console.error("ERROR GUARDAR RECTIFICATORIA LEGAL CIERRE DE COPIAS", error);
      toast.current?.show({
        severity: "error",
        summary: "Error",
  ...
  const handle_avanzar = async () => {
    ...
    try {
      ...
      console.error("ERROR AVANZAR RECTIFICATORIA LEGAL CIERRE DE COPIAS", error);
  ...
  return (
    <>
      ...
      <h2 className="text-2xl font-bold text-gray-900 mb-6">
        Rectificatoria Legal - Cierre de Copias
      </h2>
      ...
      <Accordion multiple activeIndex={[2]}>
        ...
        <AccordionTab header="Rectificatoria Legal - Cierre de Copias">
