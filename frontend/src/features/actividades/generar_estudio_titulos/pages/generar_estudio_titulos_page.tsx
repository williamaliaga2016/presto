...
import { useGenerarEstudioTitulos } from "../hooks/useGenerarEstudioTitulos";
import { useUpsertGenerarEstudioTitulos } from "../hooks/useUpsertGenerarEstudioTitulos";
import { useAvanzarGenerarEstudioTitulos } from "../hooks/useAvanzarGenerarEstudioTitulos";
import EncabezadoActividad from "@/features/encabezado/pages/EncabezadoActividad";
import FuncionesTransversales from "@/features/funciones_transversales/pages/FuncionesTransversales";

const ACTIVITY_ID = "_4X3k8H6kEeeGvo3jOJ1wYw"; // pendiente reemplazar con el ID real

const buildInitialState = (id_expediente: number): GenerarEstudioTitulos => ({
  id_generar_estudio_titulos: 0,
  id_expediente,
  observaciones: "",
  enviar_reparo: false,
  is_active: true,
...
const normalizeGenerarEstudioTitulos = (
  source: Partial<GenerarEstudioTitulos> | null | undefined,
  id_expediente_fallback: number,
): GenerarEstudioTitulos => ({
  id_generar_estudio_titulos: Number(source?.id_generar_estudio_titulos ?? 0),
...
export default function GenerarEstudioTitulosPage() {
  const toast = useRef<Toast>(null);
  const navigate = useNavigate();
  const { id_expediente: idExpedienteParam } = useParams();
  const id_expediente = Number(idExpedienteParam ?? 0);

  const [form, setForm] = useState<GenerarEstudioTitulos>(buildInitialState(id_expediente));
  ...
  const { data, isLoading } = useGenerarEstudioTitulos(id_expediente);
  const saveMutation = useUpsertGenerarEstudioTitulos();
  const avanzarMutation = useAvanzarGenerarEstudioTitulos();
  ...
  useEffect(() => {
    ...
    if (data?.status && data.detail) {
      setForm(normalizeGenerarEstudioTitulos(data.detail, id_expediente));
      setIsDisabled(Number(data.detail.id_generar_estudio_titulos) > 0);
      setCanAdvance(false);
    ...
    if (data) {
      setForm((prev) => normalizeGenerarEstudioTitulos({ ...prev, id_expediente }, id_expediente));
      setIsDisabled(false);
      setCanAdvance(false);
      hasHydratedRef.current = true;
    }
  ...
  const handleGuardar = async () => {
    ...
    try {
      setIsBusy(true);
      const payload = normalizeGenerarEstudioTitulos(
        { ...form, id_expediente },
        id_expediente,
      );
      const response = await saveMutation.mutateAsync(payload);
      if (response.status) {
        toast.current?.show({ severity: "success", summary: "Éxito", detail: "Estudio de Títulos guardado correctamente", life: 3000 });
        setForm(normalizeGenerarEstudioTitulos(response.detail ?? payload, payload.id_expediente));
        setIsDisabled(true);
        setCanAdvance(true);
        hasHydratedRef.current = true;
      } else {
  ...
  return (
    <>
      <Toast ref={toast} />
      <h2 className="text-2xl font-bold text-gray-900 mb-6">Generar Estudio de Títulos</h2>
      ...
      <Accordion activeIndex={[0, 2]} multiple>
        ...
        <AccordionTab header="Generar Estudio de Títulos">
          <Card className="w-full shadow-md card-presto-form mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              <div className="flex flex-col gap-2 md:col-span-3">
                <label className="font-semibold text-sm">¿Enviar a Reparto?</label>
