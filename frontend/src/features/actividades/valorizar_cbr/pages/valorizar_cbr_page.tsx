...
import { useAvanzarValorizarCbr } from "../hooks/useAvanzarValorizarCbr";
import { useValorizarCbr } from "../hooks/useValorizarCbr";
import { useUpsertValorizarCbr } from "../hooks/useUpsertValorizarCbr";

const ACTIVITY_ID = "_Rd1P_T9wGebOu4_W5tL2a";

const buildInitialState = (id_expediente: number): ValorizarCbr => ({
  id_valorizar_cbr: 0,
  id_expediente,
  observaciones: "",
  is_active: true,
...
const normalizeValorizarCbr = (
  source: Partial<ValorizarCbr> | null | undefined,
  id_expediente_fallback: number,
): ValorizarCbr => {
  return {
    id_valorizar_cbr: Number(source?.id_valorizar_cbr ?? 0),
...
export default function ValorizarCbrPage() {
  ...
  const [form, setForm] = useState<ValorizarCbr>(
  ...
  const { data, isLoading } = useValorizarCbr(id_expediente);
  const saveMutation = useUpsertValorizarCbr();
  const avanzarMutation = useAvanzarValorizarCbr();
  ...
  useEffect(() => {
    ...
    if (data?.status && data.detail) {
      const loadedEntity = normalizeValorizarCbr(
        data.detail,
        id_expediente,
      );

      setForm(loadedEntity);
      setIsDisabled(Number(data.detail.id_valorizar_cbr) > 0);
      setCanAdvance(false);
  ...
  const updateField = <K extends keyof ValorizarCbr>(
    field: K,
    value: ValorizarCbr[K],
  ) => {
    setForm((prev) => ({
      ...prev,
      [field]: value,
  ...
  const handleGuardar = async () => {
    ...
    try {
      ...
      const payload: ValorizarCbr = normalizeValorizarCbr(
        {
          ...form,
          id_valorizar_cbr: Number(
            form.id_valorizar_cbr ?? 0,
          ),
      ...
      if (response.status) {
        toast.current?.show({
          severity: "success",
          summary: "Éxito",
          detail: "Actividad Valorizar CBR guardada correctamente",
          life: 3000,
        });

        const savedEntity = normalizeValorizarCbr(
      ...
      console.error("ERROR GUARDAR Valorizar CBR", error);
  ...
  const handleAvanzar = async () => {
    ...
    try {
      ...
      console.error("ERROR AVANZAR Valorizar CBR", error);
  ...
  return (
    <>
      ...
      <Accordion activeIndex={[0, 2]} multiple>
        ...
        <AccordionTab header="Valorizar CBR">
