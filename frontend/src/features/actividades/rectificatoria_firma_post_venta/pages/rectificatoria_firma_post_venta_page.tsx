...
import { useRectificatoriaFirmaPostVenta } from "../hooks/useRectificatoriaFirmaPostVenta";
import { useUpsertRectificatoriaFirmaPostVenta } from "../hooks/useUpsertRectificatoriaFirmaPostVenta";
import { useAvanzarRectificatoriaFirmaPostVenta } from "../hooks/useAvanzarRectificatoriaFirmaPostVenta";
import { DataTable } from "primereact/datatable";
import { Column } from "primereact/column";
import { Calendar } from "primereact/calendar";
import { Dropdown } from "primereact/dropdown";
import { useControlesRectificatoriaFirmaPostVenta } from "../hooks/useControlesRectificatoriaFirmaPostVenta";
import { useRectificatoriaPostVentaDetalle } from "../hooks/useRectificatoriaPostVentaDetalle";
import { RectificatoriaFirmaPostVentaDetalle } from "../models/rectificatoria_firma_post_venta_detalle";

const ACTIVITY_ID = "_S4vBnL2xEepWu3_X5qK1d";

const build_initial_state = (id_expediente: number): RectificatoriaFirmaPostVenta => ({
  id_rectificatoria_firma_post_venta: 0,
  id_expediente,
...
const normalize = (
  source: Partial<RectificatoriaFirmaPostVenta> | null | undefined,
  id_expediente_fallback: number,
): RectificatoriaFirmaPostVenta => ({
  id_rectificatoria_firma_post_venta: Number(source?.id_rectificatoria_firma_post_venta ?? 0),
...
const format_date = (value: string | null | undefined): string => {
  ...
  return parsed_date.toLocaleDateString("es-CL", {
    day: "2-digit",
    month: "2-digit",
    year: "numeric",
  });
...
export default function RectificatoriaFirmaPostVentaPage() {
  ...
  const [form, set_form] = useState<RectificatoriaFirmaPostVenta>(
  ...
  const { data, isLoading } = useRectificatoriaFirmaPostVenta(id_expediente);
  const save_mutation = useUpsertRectificatoriaFirmaPostVenta();
  const avanzar_mutation = useAvanzarRectificatoriaFirmaPostVenta();
  const controlesGenerarBorradorQuery = useControlesRectificatoriaFirmaPostVenta();

  const rolesComparecenciaOptions =
  controlesGenerarBorradorQuery.data?.detail?.rolcomparecencia?.map((item) => ({
    label: item.description ?? "",
    value: item.code,
  })) ?? [];

  //bloque dropdown
  const [lockRolComparecencia, setLockRolComparecencia] = useState(false);
  //Tabla
  const [rows, setRows] = useState<RectificatoriaFirmaPostVentaDetalle[]>([]);
  const [rolComparecencia, setRolComparecencia] = useState<string | null>(null);
  const detalleQuery = useRectificatoriaPostVentaDetalle(
    id_expediente,
    rolComparecencia ?? "0",
  );

useEffect(() => {
  if (!detalleQuery.data?.status) return;

  const detalle = detalleQuery.data.detail ?? [];

  setRows(detalle);

  const firstRow = detalle[0];

  if (
    firstRow?.fecha_firma &&
    !rolComparecencia
  ) {
    setRolComparecencia(firstRow.rol_comparecencia);
    setLockRolComparecencia(true);
  }
}, [detalleQuery.data]);
  ...
  useEffect(() => {
    ...
    if (data?.status && data.detail) {
      ...
      set_form(loaded);
      set_is_disabled(Number(data.detail.id_rectificatoria_firma_post_venta) > 0);
      set_can_advance(false);
...
const handle_editar = () => {
  ...
  if (firstRow?.fecha_firma) {
    setLockRolComparecencia(true);
  } else {
    setLockRolComparecencia(false);
  }
};

  const validate_form = (): string => {
    if (!rolComparecencia || rolComparecencia === "0") {
      return "Debe seleccionar un Rol de Comparecencia.";
    }
  ...
  const handle_guardar = async () => {
    ...
    const hasFechaVacia = rows.some((x) => !x.fecha_firma);

    if (hasFechaVacia) {
      toast.current?.show({
        severity: "warn",
        summary: "Validación",
        detail: "Debe ingresar la fecha de firma para todos los registros.",
        life: 3000,
      });
    ...
    try {
        const payload: RectificatoriaFirmaPostVenta = {
          ...normalize({ ...form, id_expediente }, id_expediente),
          detalles: rows,
        };
      ...
      console.error("ERROR GUARDAR RECTIFICATORIA FIRMA", error);
  ...
  const handle_avanzar = async () => {
    ...
    try {
      ...
      console.error("ERROR AVANZAR RECTIFICATORIA FIRMA", error);
  ...
  return (
    <>
      ...
      <h2 className="text-2xl font-bold text-gray-900 mb-6">
        Rectificatoria Firma Comprador/Vendedor
      </h2>
      ...
      <Accordion multiple activeIndex={[2]}>
        ...
        <AccordionTab header="Rectificatoria Firma Comprador/Vendedor">
          <Card className="w-full shadow-md card-presto-form mb-6">
            ...
            <div className="flex flex-col gap-1 md:col-span-2">
                  ...
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <Dropdown
                      value={rolComparecencia}
                      options={rolesComparecenciaOptions}
                      optionLabel="label"
                      optionValue="value"
                      placeholder={rolesComparecenciaOptions.length === 0 ? "Cargando roles..." : "--- Selecciona ---"}
                      className="w-64"
                      onChange={(e) => setRolComparecencia(e.value)}
                      disabled={controlesGenerarBorradorQuery.isLoading || is_disabled ||
                                lockRolComparecencia}
                      showClear
                      filter
                    />
                  </div>
              ...
              >
                <Column field="relacion_titular" header="Relacion Titular" />
                ...
                <Column
                  ...
                  body={(rowData, options) => (
                    <Calendar
                      value={rowData.fecha_firma ? new Date(rowData.fecha_firma) : null}
                      onChange={(e) => {
                        ...
                        setRows((prev) =>
                          prev.map((row, i) =>
                            i === options.rowIndex
                              ? { ...row, fecha_firma: value?.toISOString() ?? null }
                              : row
                          )
                        );
                      }}
...
interface ReparoTablaProps {
  form: RectificatoriaFirmaPostVenta;
  is_disabled: boolean;
