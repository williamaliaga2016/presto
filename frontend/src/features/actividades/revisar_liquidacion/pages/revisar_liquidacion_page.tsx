...
import { useRevisarLiquidacion } from "../hooks/useRevisarLiquidacion";
import { useUpsertRevisarLiquidacion } from "../hooks/useUpsertRevisarLiquidacion";
import { useAvanzarRevisarLiquidacion } from "../hooks/useAvanzarRevisarLiquidacion";

const ACTIVITY_ID = "_Cq2A_X4hReaZa6_H5eY3n";

const build_initial_state = (
  id_expediente: number,
  id_usuario: number,
): RevisarLiquidacion => ({
  id_revisar_liquidacion: 0,
  id_expediente,
...
const normalize = (
  source: Partial<RevisarLiquidacion> | null | undefined,
  id_expediente_fallback: number,
  id_usuario_fallback: number,
): RevisarLiquidacion => ({
  id_revisar_liquidacion: Number(source?.id_revisar_liquidacion ?? 0),
...
export default function RevisarLiquidacionPage() {
  ...
  const [form, set_form] = useState<RevisarLiquidacion>(
  ...
  const { data, isLoading } = useRevisarLiquidacion(id_expediente);
  const save_mutation = useUpsertRevisarLiquidacion();
  const avanzar_mutation = useAvanzarRevisarLiquidacion();
  ...
  useEffect(() => {
    ...
    if (data?.status && data.detail) {
      ...
      set_is_disabled(Number(data.detail.id_revisar_liquidacion) > 0);
      set_can_advance(false);
    ...
    try {
      ...
      if (response.status) {
        toast.current?.show({
          severity: "success",
          summary: "Éxito",
          detail: "Revisar Liquidación guardado correctamente",
          life: 3000,
        });
  ...
  return (
    <>
      ...
      <h2 className="text-2xl font-bold text-gray-900 mb-6">Revisar Liquidación</h2>
      ...
      <Accordion multiple activeIndex={[2]}>
        ...
        <AccordionTab header="Revisar Liquidación">
            ...
            <div className="flex flex-col gap-6">
              ...
              <div className="flex flex-col gap-1">
                ...
                <InputTextarea
                  ...
                  disabled={is_disabled}
                  placeholder="Ingrese observaciones relevantes para la liquidación"
                />
              </div>
            </div>
