...
import { useReparoFormulario } from "../hooks/useReparoFormulario";
import { useUpsertReparoFormulario } from "../hooks/useUpsertReparoFormulario";
import { useAvanzarReparoFormulario } from "../hooks/useAvanzarReparoFormulario";

const ACTIVITY_ID = "_S4vBnL2xEepWu3_X5qK1d";

const build_initial_state = (id_expediente: number): ReparoFormulario => ({
  id_reparo_formulario: 0,
  id_expediente,
...
const normalize = (
  source: Partial<ReparoFormulario> | null | undefined,
  id_expediente_fallback: number,
): ReparoFormulario => ({
  id_reparo_formulario: Number(source?.id_reparo_formulario ?? 0),
...
export default function ReparoFormularioPage() {
  ...
  const [form, set_form] = useState<ReparoFormulario>(
  ...
  const { data, isLoading } = useReparoFormulario(id_expediente);
  const save_mutation = useUpsertReparoFormulario();
  const avanzar_mutation = useAvanzarReparoFormulario();
  ...
  useEffect(() => {
    ...
    if (data?.status && data.detail) {
      ...
      set_form(loaded);
      set_is_disabled(Number(data.detail.id_reparo_formulario) > 0);
      set_can_advance(false);
  ...
  const handle_guardar = async () => {
    ...
    try {
      ...
      console.error("ERROR GUARDAR REPARO FORMULARIO", error);
  ...
  const handle_avanzar = async () => {
    ...
    try {
      ...
      console.error("ERROR AVANZAR REPARO FORMULARIO", error);
  ...
  return (
    <>
      ...
      <Accordion multiple activeIndex={[2]}>
        ...
        <AccordionTab header="Reparo Formulario 2890">
