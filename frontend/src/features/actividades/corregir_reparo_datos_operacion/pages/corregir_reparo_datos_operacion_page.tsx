...
import { useCorregirReparoDatosOperacion } from "../hooks/useCorregirReparoDatosOperacion";
import { useUpsertCorregirReparoDatosOperacion } from "../hooks/useUpsertCorregirReparoDatosOperacion";
import { useAvanzarCorregirReparoDatosOperacion } from "../hooks/useAvanzarCorregirReparoDatosOperacion";

const ACTIVITY_ID = "_Zm7QxN4pEeabKf2_L9tR6w";

const build_initial_state = (id_expediente: number): CorregirReparoDatosOperacion => ({
  id_corregir_reparo_datos_operacion: 0,
  id_expediente,
...
const normalize = (
  source: Partial<CorregirReparoDatosOperacion> | null | undefined,
  id_expediente_fallback: number,
): CorregirReparoDatosOperacion => ({
  id_corregir_reparo_datos_operacion: Number(source?.id_corregir_reparo_datos_operacion ?? 0),
...
export default function CorregirReparoDatosOperacionPage() {
  ...
  const [form, set_form] = useState<CorregirReparoDatosOperacion>(build_initial_state(id_expediente));
  ...
  const { data, isLoading } = useCorregirReparoDatosOperacion(id_expediente);
  const save_mutation = useUpsertCorregirReparoDatosOperacion();
  const avanzar_mutation = useAvanzarCorregirReparoDatosOperacion();
  ...
  useEffect(() => {
    ...
    if (data?.status && data.detail) {
      ...
      set_form(loaded);
      set_is_disabled(Number(data.detail.id_corregir_reparo_datos_operacion) > 0);
      set_can_advance(false);
  ...
  const handle_guardar = async () => {
    ...
    try {
      ...
      console.error("ERROR GUARDAR CORREGIR REPARO DATOS OPERACION", error);
      toast.current?.show({
        severity: "error",
        summary: "Error",
  ...
  const handle_avanzar = async () => {
    ...
    try {
      ...
      console.error("ERROR AVANZAR CORREGIR REPARO DATOS OPERACION", error);
  ...
  return (
    <>
      ...
      <h2 className="text-2xl font-bold text-gray-900 mb-6">Corregir Reparo Datos Operación</h2>
      ...
      <Accordion multiple activeIndex={[2]}>
        ...
        <AccordionTab header="Corregir Reparo Datos Operación">
...
interface ReparoTablaProps {
  form: CorregirReparoDatosOperacion;
  is_disabled: boolean;
