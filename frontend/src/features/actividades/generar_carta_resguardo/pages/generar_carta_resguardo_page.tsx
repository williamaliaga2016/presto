...
import { useAvanzarGenerarCartaResguardo } from "../hooks/useAvanzarGenerarCartaResguardo";
import { useGenerarCartaResguardo } from "../hooks/useGenerarCartaResguardo";
import { useUpsertGenerarCartaResguardo } from "../hooks/useUpsertGenerarCartaResguardo";
import { RadioButton } from "primereact/radiobutton";
import { Dropdown } from "primereact/dropdown";
import { useControlesGenerarCartaResguardo } from "../hooks/useControlesGenerarCartaResguardo";
import { EMPTY_CONTROLES_GENERAR_cARTA_RESGUARDO } from "../models/catalogo";

const ACTIVITY_ID = "_Fm4K_N9rWeaBh7_L2tP6z";

const buildInitialState = (id_expediente: number): GenerarCartaResguardo => ({
    id_generar_carta_resguardo: 0,
    id_expediente,
    generar_carta: false,
    tipo_carta: "",
...
const normalizeGenerarCartaResguardo = (
    source: Partial<GenerarCartaResguardo> | null | undefined,
    id_expediente_fallback: number,
): GenerarCartaResguardo => {
    return {
        id_generar_carta_resguardo: Number(source?.id_generar_carta_resguardo ?? 0),
        id_expediente: Number(source?.id_expediente ?? id_expediente_fallback ?? 0),
        generar_carta: Boolean(source?.generar_carta ?? false),
        tipo_carta: source?.tipo_carta ?? "",
...
export default function GenerarCartaResguardoPage() {
    ...
    const [form, setForm] = useState<GenerarCartaResguardo>(
    ...
    const { data, isLoading } = useGenerarCartaResguardo(id_expediente);
    const saveMutation = useUpsertGenerarCartaResguardo();
    const avanzarMutation = useAvanzarGenerarCartaResguardo();
    const controlesGenerarCartaResguardoQuery = useControlesGenerarCartaResguardo();
    
    const controlesGenerarCartaResguardo = controlesGenerarCartaResguardoQuery.data?.status
        ? controlesGenerarCartaResguardoQuery.data.detail ?? EMPTY_CONTROLES_GENERAR_cARTA_RESGUARDO
        : EMPTY_CONTROLES_GENERAR_cARTA_RESGUARDO;
    ...
    useEffect(() => {
        ...
        if (data?.status && data.detail) {
            const loadedEntity = normalizeGenerarCartaResguardo(
                data.detail,
                id_expediente,
            );

            setForm(loadedEntity);
            setIsDisabled(Number(data.detail.id_generar_carta_resguardo) > 0);
            setCanAdvance(false);
        ...
        if (data) {
            setForm((prev) =>
                normalizeGenerarCartaResguardo(
                    {
                        ...prev,
                        id_expediente,
                    },
    ...
    const updateField = <K extends keyof GenerarCartaResguardo>(
        field: K,
        value: GenerarCartaResguardo[K],
    ) => {
        setForm((prev) => ({
            ...prev,
            [field]: value,
    ...
    const handleGuardar = async () => {
        ...
        try {
            ...
            const payload: GenerarCartaResguardo = normalizeGenerarCartaResguardo(
                {
                    ...form,
                    id_generar_carta_resguardo: Number(
                        form.id_generar_carta_resguardo ?? 0,
                    ),
            ...
            if (response.status) {
                ...
                const savedEntity = normalizeGenerarCartaResguardo(
                ...
                <AccordionTab header="Generar Carta Resguardo">
                    <Card className="w-full shadow-md card-presto-form mb-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                            ...
                            <div className="flex flex-col gap-1">
                                <label className="font-semibold text-sm">Generar Carta Resguardo *</label>

                                <div className="flex items-center gap-6 h-10">
                                    <div className="flex items-center gap-2">
                                        <RadioButton
                                            className="form-radio-presto"
                                            inputId="generar_carta_si"
                                            checked={form?.generar_carta === true}
                                            disabled={isDisabled}
                                            onChange={() => updateField('generar_carta', true)}
                                        />

                                        <label
                                            htmlFor="generar_carta_si"
                                    ...
                                    <div className="flex items-center gap-2">
                                        <RadioButton
                                            className="form-radio-presto"
                                            inputId="generar_carta_no"
                                            checked={form?.generar_carta === false}
                                            disabled={isDisabled}
                                            onChange={() => updateField('generar_carta', false)}
                                        />

                                        <label
                                            htmlFor="generar_carta_no"
                                            className="text-sm text-gray-700"
                                        >
                                            No
                                        </label>
                            ...
                            <div className="flex flex-col gap-1">
                                <label className="font-semibold text-sm">Tipo de carta resguardo</label>
                                <Dropdown
                                    value={form?.tipo_carta ?? null}
                                    options={controlesGenerarCartaResguardo.tipocarta}
                                    optionLabel="description"
                                    optionValue="code"
                                    disabled={isDisabled}
                                    className="form-dropdown-presto w-full"
                                    placeholder="Seleccione"
                                    onChange={(e) => updateField('tipo_carta', e.value)}
                                    showClear
                                />
                            </div>
