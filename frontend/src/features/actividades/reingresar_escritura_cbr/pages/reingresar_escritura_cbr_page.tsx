...
import { useReingresarEscrituraCbr } from "../hooks/useReingresarEscrituraCbr";
import { useUpsertReingresarEscrituraCbr } from "../hooks/useUpsertReingresarEscrituraCbr";
import { useAvanzarReingresarEscrituraCbr } from "../hooks/useAvanzarReingresarEscrituraCbr";

const ACTIVITY_ID = "_Xk3V_S7cMeaUv6_C7zT4g";

const build_initial_state = (id_expediente: number): ReingresarEscrituraCbr => ({
  id_reingresar_escritura_cbr: 0,
  id_expediente,
...
const normalize = (
  source: Partial<ReingresarEscrituraCbr> | null | undefined,
  id_expediente_fallback: number,
): ReingresarEscrituraCbr => ({
  id_reingresar_escritura_cbr: Number(source?.id_reingresar_escritura_cbr ?? 0),
...
export default function ReingresarEscrituraCbrPage() {
  ...
  const [form, set_form] = useState<ReingresarEscrituraCbr>(build_initial_state(id_expediente));
  ...
  const { data, isLoading } = useReingresarEscrituraCbr(id_expediente);
  const save_mutation = useUpsertReingresarEscrituraCbr();
  const avanzar_mutation = useAvanzarReingresarEscrituraCbr();
  ...
  useEffect(() => {
    ...
    if (data?.status && data.detail) {
      ...
      set_form(loaded);
      set_is_disabled(Number(data.detail.id_reingresar_escritura_cbr) > 0);
      set_can_advance(false);
