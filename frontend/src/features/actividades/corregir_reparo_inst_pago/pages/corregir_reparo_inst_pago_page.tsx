...
import { useCorregirReparoInstPago } from "../hooks/useCorregirReparoInstPago";
import { useUpsertCorregirReparoInstPago } from "../hooks/useUpsertCorregirReparoInstPago";
import { useAvanzarCorregirReparoInstPago } from "../hooks/useAvanzarCorregirReparoInstPago";

const ACTIVITY_ID = "_Ao4Y_V8fPeaXy7_F8cW5k";

const build_initial_state = (id_expediente: number): CorregirReparoInstPago => ({
  id_corregir_reparo_inst_pago: 0,
  id_expediente,
...
const normalize = (
  source: Partial<CorregirReparoInstPago> | null | undefined,
  id_expediente_fallback: number,
): CorregirReparoInstPago => ({
  id_corregir_reparo_inst_pago: Number(source?.id_corregir_reparo_inst_pago ?? 0),
  ...
  enviar_a_reparo: source?.enviar_a_reparo ?? true,
...
export default function CorregirReparoInstPagoPage() {
  ...
  const [form, set_form] = useState<CorregirReparoInstPago>(build_initial_state(id_expediente));
  ...
  const { data, isLoading } = useCorregirReparoInstPago(id_expediente);
  const save_mutation = useUpsertCorregirReparoInstPago();
  const avanzar_mutation = useAvanzarCorregirReparoInstPago();
  ...
  useEffect(() => {
    ...
    if (data?.status && data.detail) {
      ...
      set_form(loaded);
      set_is_disabled(Number(data.detail.id_corregir_reparo_inst_pago) > 0);
      set_can_advance(false);
  ...
  const validate_form = (): string => {
    ...
    const updateField = <K extends keyof CorregirReparoInstPago>(
        field: K,
        value: CorregirReparoInstPago[K],
    ) => {
        set_form((prev) => ({
            ...prev,
            [field]: value,
  ...
  return (
    <>
      ...
      <h2 className="text-2xl font-bold text-gray-900 mb-6">Corregir Reparo Inst Pago</h2>
