...
import { useAvanzarVisarOperacion } from "../hooks/useAvanzarVisarOperacion";
import { useVisarOperacion } from "../hooks/useVisarOperacion";
import { useUpsertVisarOperacion } from "../hooks/useUpsertVisarOperacion";

const ACTIVITY_ID = "_F5kTyM7wEeoJq8_Y2nV6p";

const buildInitialState = (id_expediente: number): VisarOperacion => ({
  id_visar_operacion: 0,
  id_expediente,
...
const normalizeVisarOperacion = (
  source: Partial<VisarOperacion> | null | undefined,
  id_expediente_fallback: number,
): VisarOperacion => {
  return {
    id_visar_operacion: Number(source?.id_visar_operacion ?? 0),
...
export default function VisarOperacionPage() {
  ...
  const [form, setForm] = useState<VisarOperacion>(
  ...
  const { data, isLoading } = useVisarOperacion(id_expediente);
  const saveMutation = useUpsertVisarOperacion();
  const avanzarMutation = useAvanzarVisarOperacion();
  ...
  useEffect(() => {
    ...
    if (data?.status && data.detail) {
      const loadedEntity = normalizeVisarOperacion(
        data.detail,
        id_expediente,
      );

      setForm(loadedEntity);
      setIsDisabled(Number(data.detail.id_visar_operacion) > 0);
      setCanAdvance(false);
  ...
  const updateField = <K extends keyof VisarOperacion>(
    field: K,
    value: VisarOperacion[K],
  ) => {
    setForm((prev) => ({
      ...prev,
      [field]: value,
  ...
  const handleGuardar = async () => {
    ...
    try {
      ...
      const payload: VisarOperacion = normalizeVisarOperacion(
        {
          ...form,
          id_visar_operacion: Number(
            form.id_visar_operacion ?? 0,
          ),
      ...
      if (response.status) {
        toast.current?.show({
          severity: "success",
          summary: "Éxito",
          detail: "Actividad Visar Operación guardada correctamente",
          life: 3000,
        });

        const savedEntity = normalizeVisarOperacion(
      ...
      console.error("ERROR GUARDAR VISAR OPERACIÓN", error);
  ...
  const handleAvanzar = async () => {
    ...
    try {
      ...
      console.error("ERROR AVANZAR VISAR OPERACIÓN", error);
  ...
  return (
    <>
      ...
      <Accordion activeIndex={[0, 2]} multiple>
        ...
        <AccordionTab header="Visar Operación">
          <Card className="w-full shadow-md card-presto-form mb-6">
            ...
            <div className="flex flex-col gap-2 md:col-span-3 mb-4">
