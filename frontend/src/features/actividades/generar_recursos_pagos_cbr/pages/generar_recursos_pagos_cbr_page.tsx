...
import { useAvanzarGenerarRecursosPagosCbr } from "../hooks/useAvanzarGenerarRecursosPagosCbr";
import { useGenerarRecursosPagosCbr } from "../hooks/useGenerarRecursosPagosCbr";
import { useUpsertGenerarRecursosPagosCbr } from "../hooks/useUpsertGenerarRecursosPagosCbr";

const ACTIVITY_ID = "_Se4Q_M3xHebPv7_X8uN5b";

const buildInitialState = (id_expediente: number): GenerarRecursosPagosCbr => ({
  id_generar_recursos_pagos_cbr: 0,
  id_expediente,
  observaciones: "",
  is_active: true,
  row_status: true,
...
const normalizeGenerarRecursosPagosCbr = (
  source: Partial<GenerarRecursosPagosCbr> | null | undefined,
  id_expediente_fallback: number,
): GenerarRecursosPagosCbr => {
  return {
    id_generar_recursos_pagos_cbr: Number(source?.id_generar_recursos_pagos_cbr ?? 0),
...
export default function GenerarRecursosPagosCbrPage() {
  ...
  const [form, setForm] = useState<GenerarRecursosPagosCbr>(
  ...
  const { data, isLoading } = useGenerarRecursosPagosCbr(id_expediente);
  const saveMutation = useUpsertGenerarRecursosPagosCbr();
  const avanzarMutation = useAvanzarGenerarRecursosPagosCbr();
  ...
  useEffect(() => {
    ...
    if (data?.status && data.detail) {
      const loadedEntity = normalizeGenerarRecursosPagosCbr(
        data.detail,
        id_expediente,
      );

      setForm(loadedEntity);
      setIsDisabled(Number(data.detail.id_generar_recursos_pagos_cbr) > 0);
      setCanAdvance(false);
    ...
    if (data) {
      setForm((prev) =>
        normalizeGenerarRecursosPagosCbr(
          {
            ...prev,
            id_expediente,
          },
  ...
  const updateField = <K extends keyof GenerarRecursosPagosCbr>(
    field: K,
    value: GenerarRecursosPagosCbr[K],
  ) => {
    setForm((prev) => ({
      ...prev,
      [field]: value,
  ...
  const handleGuardar = async () => {
    ...
    try {
      ...
      const payload: GenerarRecursosPagosCbr = normalizeGenerarRecursosPagosCbr(
        {
          ...form,
          id_generar_recursos_pagos_cbr: Number(
            form.id_generar_recursos_pagos_cbr ?? 0,
          ),
      ...
      if (response.status) {
        toast.current?.show({
          severity: "success",
          summary: "Éxito",
          detail: "Actividad Generar recursos pagos CBR guardada correctamente",
          life: 3000,
        });

        const savedEntity = normalizeGenerarRecursosPagosCbr(
      ...
      console.error("ERROR GUARDAR Generar recursos pagos CBR", error);
  ...
  const handleAvanzar = async () => {
    ...
    try {
      ...
      console.error("ERROR AVANZAR Generar recursos pagos CBR", error);
  ...
  return (
    <>
      ...
      <Accordion activeIndex={[0, 2]} multiple>
        ...
        <AccordionTab header="Generar recursos pagos CBR">
