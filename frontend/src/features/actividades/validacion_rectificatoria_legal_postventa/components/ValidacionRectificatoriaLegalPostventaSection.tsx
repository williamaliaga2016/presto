...
import type { ValidacionRectificatoriaLegalPostventaDatosPersonales } from '../models/validacion_rectificatoria_legal_postventa';

interface ValidacionRectificatoriaLegalPostventaSectionProps {
  value: ValidacionRectificatoriaLegalPostventaDatosPersonales[];
  idExpediente: number;
  idValidacionRectificatorialegalPostventa: number;
  disabled: boolean;
  /**
   ...
   */
  canEdit?: boolean;
  controles: ControlesValidacionRectificatoriaLegalPostventa;
  loadingControles?: boolean;
  onChange: (value: ValidacionRectificatoriaLegalPostventaDatosPersonales[]) => void;
...
export default function ValidacionRectificatoriaLegalPostventaSection({
  value,
  idExpediente,
  ...
  onChange,
  onWarn,
}: ValidacionRectificatoriaLegalPostventaSectionProps) {
  const [dialogVisible, setDialogVisible] = useState(false);
  const [dialogMode, setDialogMode] = useState<DialogMode>('view');
  const [editingIndex, setEditingIndex] = useState<number | null>(null);
  const [draft, setDraft] = useState<ValidacionRectificatoriaLegalPostventaDatosPersonales>(
    emptyFaltantesFirma(idExpediente, idValidacionRectificatorialegalPostventa),
  );
  ...
  const openNew = () => {
    ...
    setDraft(emptyFaltantesFirma(idExpediente, idValidacionRectificatorialegalPostventa));
    setDialogVisible(true);
  };

  const openView = (rowData: ValidacionRectificatoriaLegalPostventaDatosPersonales) => {
    setEditingIndex(null);
    setDialogMode('view');
    setDraft({
      ...emptyFaltantesFirma(idExpediente, idValidacionRectificatorialegalPostventa),
      ...
      id_validacion_rectificatoria_legal_postventa:
        rowData.id_validacion_rectificatoria_legal_postventa || idValidacionRectificatorialegalPostventa,
    });
    setDialogVisible(true);
  };

  const openEdit = (
    rowData: ValidacionRectificatoriaLegalPostventaDatosPersonales,
    rowIndex: number,
  ) => {
    if (!canModifyRows) return;
  ...
  const updateDraft = <K extends keyof ValidacionRectificatoriaLegalPostventaDatosPersonales>(
    field: K,
    fieldValue: ValidacionRectificatoriaLegalPostventaDatosPersonales[K],
  ) => {
  ...
  const saveDraft = () => {
    ...
    const normalizedDraft: ValidacionRectificatoriaLegalPostventaDatosPersonales = {
      ...draft,
      id_expediente: idExpediente || draft.id_expediente,
      id_validacion_rectificatoria_legal_postventa:
        idValidacionRectificatorialegalPostventa || draft.id_validacion_rectificatoria_legal_postventa,
  ...
  const executeRemoveRow = (
    ...
    if (rowData.id_validacion_rectificatoria_legal_postventa_datos_personales > 0) {
      onChange(
        (value ?? []).map((item, index) =>
          index === rowIndex
            ? {
    ...
    <div className="flex flex-col gap-4">
      ...
      >
        <Column
          ...
          body={(rowData: ValidacionRectificatoriaLegalPostventaDatosPersonales) =>
