...
import { useRectificatoriaLegalCartaResguardo } from "../hooks/useRectificatoriaLegalCartaResguardo";
import { useUpsertRectificatoriaLegalCartaResguardo } from "../hooks/useUpsertRectificatoriaLegalCartaResguardo";
import { useAvanzarRectificatoriaLegalCartaResguardo } from "../hooks/useAvanzarRectificatoriaLegalCartaResguardo";

const ACTIVITY_ID = "";

const build_initial_state = (id_expediente: number): RectificatoriaLegalCartaResguardo => ({
  id_rectificatoria_legal_carta_resguardo: 0,
  id_expediente,
...
const normalize = (
  source: Partial<RectificatoriaLegalCartaResguardo> | null | undefined,
  id_expediente_fallback: number,
): RectificatoriaLegalCartaResguardo => ({
  id_rectificatoria_legal_carta_resguardo: Number(source?.id_rectificatoria_legal_carta_resguardo ?? 0),
...
export default function RectificatoriaLegalCartaResguardoPage() {
  ...
  const [form, set_form] = useState<RectificatoriaLegalCartaResguardo>(build_initial_state(id_expediente));
  ...
  const { data, isLoading } = useRectificatoriaLegalCartaResguardo(id_expediente);
  const save_mutation = useUpsertRectificatoriaLegalCartaResguardo();
  const avanzar_mutation = useAvanzarRectificatoriaLegalCartaResguardo();
  ...
  useEffect(() => {
    ...
    if (data?.status && data.detail) {
      ...
      set_form(loaded);
      set_is_disabled(Number(data.detail.id_rectificatoria_legal_carta_resguardo) > 0);
      set_can_advance(false);
  ...
  const handle_guardar = async () => {
    ...
    try {
      ...
      console.error("ERROR GUARDAR RECTIFICATORIA LEGAL CARTA RESGUARDO", error);
      toast.current?.show({
        severity: "error",
        summary: "Error",
  ...
  const handle_avanzar = async () => {
    ...
    try {
      ...
      console.error("ERROR AVANZAR RECTIFICATORIA LEGAL CARTA RESGUARDO", error);
  ...
  return (
    <>
      ...
      <h2 className="text-2xl font-bold text-gray-900 mb-6">
        Rectificatoria Legal - Carta de Resguardo
      </h2>
      ...
      <Accordion multiple activeIndex={[2]}>
        ...
        <AccordionTab header="Rectificatoria Legal - Carta de Resguardo">
