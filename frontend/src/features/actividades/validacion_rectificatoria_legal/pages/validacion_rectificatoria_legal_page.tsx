...
import ValidacionRectificatoriaLegalSection from '../components/ValidacionRectificatoriaLegalSection';
import { useAvanzarValidacionRectificatoriaLegal } from '../hooks/useAvanzarValidacionRectificatoriaLegal';
import { useValidacionRectificatoriaLegal } from '../hooks/useValidacionRectificatoriaLegal';
import { useControlesValidacionRectificatoriaLegal } from '../hooks/useControlesValidacionRectificatoriaLegal';
import { useUpsertValidacionRectificatoriaLegal } from '../hooks/useUpsertValidacionRectificatoriaLegal';
import {
    EMPTY_CONTROLES_VALIDACION_RECTIFICATORIA_LEGAL,
    type ControlesValidacionRectificatoriaLegal
...
const buildValidacionRectificatoriaLegalDatosPersonalesInitialState = (
...
const buildInitialState = (id_expediente: number): ValidacionRectificatoriaLegal => ({
    id_validacion_rectificatoria_legal: 0,
    id_expediente,
    ...
    validacion_rectificatoria_legal_datos_personales: [buildValidacionRectificatoriaLegalDatosPersonalesInitialState(id_expediente)],
});



const normalizeValidacionRectificatoriaLegalDatosPersonales = (
    source: Partial<ValidacionRectificatoriaLegalDatosPersonales> | null | undefined,
    id_expediente_fallback: number,
    id_validacion_rectificatoria_legal_fallback = 0,
): ValidacionRectificatoriaLegalDatosPersonales => ({
    ...buildValidacionRectificatoriaLegalDatosPersonalesInitialState(
        Number(source?.id_expediente ?? id_expediente_fallback ?? 0),
        Number(source?.id_validacion_rectificatoria_legal ?? id_validacion_rectificatoria_legal_fallback ?? 0),
    ),
    id_validacion_rectificatoria_legal_datos_personales: Number(
        source?.id_validacion_rectificatoria_legal_datos_personales ?? 0,
    ),
    id_validacion_rectificatoria_legal: Number(
        source?.id_validacion_rectificatoria_legal ?? id_validacion_rectificatoria_legal_fallback ?? 0,
    ),
...
const normalizeValidacionRectificatoriaLegalDatoPersonal = (
    source: Partial<ValidacionRectificatoriaLegalDatosPersonales>[] | null | undefined,
    id_expediente_fallback: number,
    id_validacion_rectificatoria_legal_fallback = 0,
): ValidacionRectificatoriaLegalDatosPersonales[] =>
    (source ?? []).map((vendedor) =>
        normalizeValidacionRectificatoriaLegalDatosPersonales(
            vendedor,
            id_expediente_fallback,
            id_validacion_rectificatoria_legal_fallback,
        ),
    );



const normalizeValidacionRectificatoriaLegal = (
    source: Partial<ValidacionRectificatoriaLegal> | null | undefined,
    id_expediente_fallback: number,
): ValidacionRectificatoriaLegal => {
    const idValidacionRectificatoriaLegal = Number(source?.id_validacion_rectificatoria_legal ?? 0);
    ...
    return {
        ...
        validacion_rectificatoria_legal_datos_personales: normalizeValidacionRectificatoriaLegalDatoPersonal(
            source?.validacion_rectificatoria_legal_datos_personales,
            idExpediente,
...
export default function ValidacionRectificatoriaLegalPage() {
    ...
    const [form, setForm] = useState<ValidacionRectificatoriaLegal>(
    ...
    const [seleccionados, setSeleccionados] = useState<
        ValidacionRectificatoriaLegalDatosPersonales[]
    >([]);
    const { data, isLoading } = useValidacionRectificatoriaLegal(id_expediente);
    const saveMutation = useUpsertValidacionRectificatoriaLegal();
    const avanzarMutation = useAvanzarValidacionRectificatoriaLegal();
    ...
    const controlesValidacionRectificatoriaLegalQuery = useControlesValidacionRectificatoriaLegal();

    const catalogo: ControlesValidacionRectificatoriaLegal = useMemo(() => {
        if (!controlesValidacionRectificatoriaLegalQuery.data?.status) {
            return EMPTY_CONTROLES_VALIDACION_RECTIFICATORIA_LEGAL;
        }

        return (
            controlesValidacionRectificatoriaLegalQuery.data.detail ??
            EMPTY_CONTROLES_VALIDACION_RECTIFICATORIA_LEGAL
        );
    }, [controlesValidacionRectificatoriaLegalQuery.data]);
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        if (data?.status && data.detail) {
            setForm(normalizeValidacionRectificatoriaLegal(data.detail, id_expediente));
            setIsDisabled(Number(data.detail.id_validacion_rectificatoria_legal) > 0);
            setCanAdvance(false);
            hasHydratedRef.current = true;
            return;
        }
    ...
    const updateValidacionRectificatoriaLegalDatosPersonales = (
        validacion_rectificatoria_legal_datos_personales: ValidacionRectificatoriaLegalDatosPersonales[],
    ) => {
        setForm((prev) => ({
            ...prev,
            validacion_rectificatoria_legal_datos_personales: normalizeValidacionRectificatoriaLegalDatoPersonal(
                ...
                prev.id_expediente,
                prev.id_validacion_rectificatoria_legal,
            ),
        }));
    };
    const handleAgregarSeleccionados = () => {
        const existentes =
            form.validacion_rectificatoria_legal_datos_personales ?? [];
        ...
        updateValidacionRectificatoriaLegalDatosPersonales([
            ...existentes,
            ...nuevos.map((x) => ({
                ...x,
    ...
    const buildPayload = () => {
        ...
        const idValidacionRectificatoriaLegal = Number(form.id_validacion_rectificatoria_legal ?? 0);

        return normalizeValidacionRectificatoriaLegal(
            {
                ...form,
    ...
    const handleGuardar = async () => {
        ...
        try {
            ...
            if (response.status) {
                const savedEntity = normalizeValidacionRectificatoriaLegal(
            ...
            <Accordion activeIndex={[0, 2]} multiple>
                ...
                <AccordionTab header="Validación Rectificatoria Legal">
                    ...
                    <Card className="w-full shadow-md card-presto-form mb-6">
                        ...
                        <ValidacionRectificatoriaLegalSection
                            key={`antecedentes-comprador-${canEditAntecedentesComprador ? 'edit' : 'view'}`}
                            value={form.validacion_rectificatoria_legal_datos_personales ?? []}
                            idExpediente={Number(form.id_expediente || id_expediente || 0)}
                            idValidacionRectificatorialegal={Number(form.id_validacion_rectificatoria_legal ?? 0)}
                            disabled={isDisabled || isBusy}
                            canEdit={canEditAntecedentesComprador}
                            controles={catalogo}
                            loadingControles={isLoading}
                            onChange={updateValidacionRectificatoriaLegalDatosPersonales}
                            onWarn={(message) =>
                                toast.current?.show({
                                    severity: 'warn',
                                    summary: 'Validación',
                        ...
                        <div className="form-actions">
                            ...
                            <Button
                                ...
                                disabled={isBusy || (!canAdvance && !form.id_validacion_rectificatoria_legal)}
                                className="btn-responsive"
                            />
...
interface ReparoTablaProps {
    form: ValidacionRectificatoriaLegal;
    is_disabled: boolean;
