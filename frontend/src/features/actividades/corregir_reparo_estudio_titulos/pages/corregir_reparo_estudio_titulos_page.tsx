...
import { useCorregirReparoEstudioTitulos } from "../hooks/useCorregirReparoEstudioTitulos";
import { useUpsertCorregirReparoEstudioTitulos } from "../hooks/useUpsertCorregirReparoEstudioTitulos";
import { useAvanzarCorregirReparoEstudioTitulos } from "../hooks/useAvanzarCorregirReparoEstudioTitulos";

const ACTIVITY_ID = "_M4pKsZ2uEegQn8_X7bR1h";

const build_initial_state = (id_expediente: number): CorregirReparoEstudioTitulos => ({
  id_corregir_reparo_estudio_titulos: 0,
  id_expediente,
...
const normalize = (
  source: Partial<CorregirReparoEstudioTitulos> | null | undefined,
  id_expediente_fallback: number,
): CorregirReparoEstudioTitulos => ({
  id_corregir_reparo_estudio_titulos: Number(
    source?.id_corregir_reparo_estudio_titulos ?? 0,
  ),
...
export default function CorregirReparoEstudioTitulosPage() {
  ...
  const [form, set_form] = useState<CorregirReparoEstudioTitulos>(
  ...
  const { data, isLoading } = useCorregirReparoEstudioTitulos(id_expediente);
  const save_mutation = useUpsertCorregirReparoEstudioTitulos();
  const avanzar_mutation = useAvanzarCorregirReparoEstudioTitulos();
  ...
  useEffect(() => {
    ...
    if (data?.status && data.detail) {
      ...
      set_form(loaded);
      set_is_disabled(Number(data.detail.id_corregir_reparo_estudio_titulos) > 0);
      set_can_advance(false);
  ...
  const handle_guardar = async () => {
    ...
    try {
      ...
      console.error("ERROR GUARDAR CORREGIR REPARO ESTUDIO TÍTULOS", error);
      toast.current?.show({
        severity: "error",
        summary: "Error",
  ...
  const handle_avanzar = async () => {
    ...
    try {
      ...
      console.error("ERROR AVANZAR CORREGIR REPARO ESTUDIO TÍTULOS", error);
  ...
  return (
    <>
      ...
      <h2 className="text-2xl font-bold text-gray-900 mb-6">
        Corregir Reparo Estudio de Títulos
      </h2>
      ...
      <Accordion multiple activeIndex={[2]}>
        ...
        <AccordionTab header="Corregir Reparo Estudio de Títulos">
...
interface ReparoTablaProps {
  form: CorregirReparoEstudioTitulos;
  is_disabled: boolean;
