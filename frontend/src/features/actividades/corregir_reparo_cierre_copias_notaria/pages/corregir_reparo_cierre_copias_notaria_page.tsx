...
import { useCorregirReparoCierreCopiasNotaria } from "../hooks/useCorregirReparoCierreCopiasNotaria";
import { useUpsertCorregirReparoCierreCopiasNotaria} from "../hooks/useUpsertCorregirReparoCierreCopiasNotaria";
import { useAvanzarCorregirReparoCierreCopiasNotaria } from "../hooks/useAvanzarCorregirReparoCierreCopiasNotaria";

const ACTIVITY_ID = "_Gy7A_X3wSebTb1_M6eY8t";

const build_initial_state = (id_expediente: number): CorregirReparoCierreCopiasNotaria => ({
  id_corregir_reparo_cierre_copias_notaria: 0,
  id_expediente,
...
const normalize = (
  source: Partial<CorregirReparoCierreCopiasNotaria> | null | undefined,
  id_expediente_fallback: number,
): CorregirReparoCierreCopiasNotaria => ({
  id_corregir_reparo_cierre_copias_notaria: Number(source?.id_corregir_reparo_cierre_copias_notaria ?? 0),
...
export default function CorregirReparoCierreCopiasNotariaPage() {
  ...
  const [form, set_form] = useState<CorregirReparoCierreCopiasNotaria>(build_initial_state(id_expediente));
  ...
  const { data, isLoading } = useCorregirReparoCierreCopiasNotaria(id_expediente);
  const save_mutation = useUpsertCorregirReparoCierreCopiasNotaria();
  const avanzar_mutation = useAvanzarCorregirReparoCierreCopiasNotaria();
  ...
  useEffect(() => {
    ...
    if (data?.status && data.detail) {
      ...
      set_form(loaded);
      set_is_disabled(Number(data.detail.id_corregir_reparo_cierre_copias_notaria) > 0);
      set_can_advance(false);
  ...
  const handle_guardar = async () => {
    ...
    try {
      ...
      console.error("ERROR GUARDAR CORREGIR REPARO FABRICA", error);
      toast.current?.show({
        severity: "error",
        summary: "Error",
  ...
  const handle_avanzar = async () => {
    ...
    try {
      ...
      console.error("ERROR AVANZAR CORREGIR REPARO FABRICA", error);
  ...
  return (
    <>
      ...
      <h2 className="text-2xl font-bold text-gray-900 mb-6">Corregir Reparo Fábrica</h2>
      ...
      <Accordion multiple activeIndex={[2]}>
        ...
        <AccordionTab header="Corregir Reparo Fábrica">
...
interface ReparoTablaProps {
  form: CorregirReparoCierreCopiasNotaria;
  is_disabled: boolean;
